...
man ρ = 0.976, 100% accuracy under UCCP normalization). Neuromorphic hardware
(Intel Loihi-2) is separately classified from binary-digital ANN, confirming the α-barrier
prediction. Intelligence Efficiency ηI reveals an approximately six-order-of-magnitude
...
7
2.2.1
Derivation of Universal Thresholds via Symmetry Breaking . . . . . .
8
2.3
Cross-System Validation . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
9
2.4
The Triple Lock and the Thermodynamic Asymptote . . . . . . . . . . . . .
13
3
Discussion
14
...
15
3.3
Convergence of AI Architecture toward CST . . . . . . . . . . . . . . . . . .
17
3.4
...
18
4.1
Data Provenance and Quality Grading
. . . . . . . . . . . . . . . . . . . . .
18
4.2
Unified Cross-Species Computation Protocol (UCCP) . . . . . . . . . . . . .
18
4.3
...
pass most humans on standardized tests in law, medicine, and coding.
Yet as we show
...
the coupling Γst between them—the degree to which the network’s functional dynamics are
...
modest complexity.
But when structure and function are physically coupled, each rein-
...
theory (Axiom 5).
Axiom 1 (Boundedness): 0 < Sc, Tc ≤1; Γst ∈[−1, 1]. Axiom 2 (Monotonicity): CST
is strictly monotonically increasing in Sc, Tc, and Γst when Γst ≥0. Axiom 3 (Coupling
...
CST = (Sc · Tc) · exp(α · Γst)
(1)
Spatial complexity Sc quantifies structural integration potential as the geometric mean
...
Tc = (λeff · Φ · Ψ · Θ)1/4
(3)
where λeff is the neural avalanche branching ratio [4]; Φ is inter-regional phase synchrony; Ψ
...
(4)
6
Intelligence Efficiency ηI extends CST to a sustainability metric:
ηI = CST/Pnorm
(5)
...
and functional freedom. The human brain achieves Γst ≈0.39–0.45, approaching but not
reaching this theoretical optimum.
Figure 1: CST framework decomposition. Four components of equation (1): Sc (struc-
tural topology), Tc (dynamical richness), Γst (structure–function coupling), and α (device
...
from the α-determined exponential term (×5.8 from α alone, ×12.3 from Γst coupling).
2.2
Six-Level Intelligence Hierarchy
We propose that intelligence emerges in discrete levels at six fundamental mathematical
constants (Table 1).
Each threshold corresponds to a distinct symmetry-breaking phase
transition: 1/
√
2 is the coherent signal propagation threshold (3dB analog); 1 is the unit
eigenvalue for persistent memory traces; φ arises from Fibonacci-type recursive connectivity;
e is the natural growth rate eigenvalue for learning dynamics [21]; π marks onset of stable
7
...
are not empirical fits. They are analytically derived from consecutive symmetry-breaking
...
coupling term collapses to unity, yielding CST = Sc·Tc with no exponential amplification and
...
(NMH): triangles (orange).
Six horizontal dashed lines mark the intelligence thresholds
{1/
√
...
Reflexive
T1
B12 BNN Marmoset cortex
∼636M 0.783 0.748 0.42
3.91
3.0260V.
Gen-
eral
T1
B13 BNN Bumblebee MB
∼1M
0.598 0.564 0.30
3.47
0.9552I. Percep-
...
Level
Data
B15 BNN Pigeon pallium
∼310M 0.671 0.658 0.38
3.91
1.9508IV.
Cre-
ative
...
cortex
(Eptesicus)
∼500M 0.698 0.679 0.39
3.91
2.1776IV.
Cre-
ative
T1
B18 BNN Zebra finch cortex
∼300M 0.651 0.631 0.37
3.91
1.7454III.
Cre-
ativity
...
tex)
∼76M 0.721 0.709 0.42
3.91
3.2764V.
Gen-
eral
T1
B20 BNN Zebrafish (Adult)
∼10M 0.641 0.623 0.37
3.91
2.2990IV.
Cre-
ative
...
T1
A18 NMH†SpiNNaker2
∼144M 0.571 0.548 0.30
3.47
1.1190II.
Reac-
tion
T1
A19 NMH†BrainScaleS-2
∼512
0.542 0.516 0.28
3.47
0.9823I. Percep-
...
13
The binary-digital ceiling: CSTemergent,max ≈0.35 (at Γst →0.5, αdigital = 0.69)—
...
Middle (Γst-lock): Γst frozen at training. Innermost (Ψ-lock): inference-time weight freezing
...
IIL = CSTspecies = (Sc · Tc) · exp(α · Γst)
TILtask = CSTspecies · exp(α · ∆Γstexpertise)
Eenv
where Eenv represents the irreducible complexity (thermodynamic entropy lower bound) of
the target task.
The six-order-of-magnitude ηI gap between biological and binary-digital AI is not an
...
Γst arises from material physics: synaptic STDP continuously aligns structural connectivity
with functional experience. GPT-class inference requires ∼300 kW to maintain a frozen Γst
...
MoE (ηI = 3.0 × 10−6); MLP (ηI = 4.6 × 10−7).
Six-order-of-magnitude gap between
...
the Γst barrier through materials, not algorithms. The required transitions are concrete and
15
staged (Table 3):
...
(Integration, 2026–2028): SDSoC integration, targets L2–L3.
Gen3 (SDI Coordination,
...
scatter plot of 20 ANN architectures, x-axis = year of publication, y-axis = CSTemergent.
Color encodes dominant architectural innovation (Sc: blue, Tc: green, Γst: orange). All
...
3.5
Limitations
Γst comparability across BNN and ANN measurement modalities is validated within ±0.04
for C. elegans simulations; domain-specific CSTfunc estimates for modern LLMs are theo-
...
17
idate CST in deployed neuromorphic hardware, extend ηI measurements to frontier LLMs
with disclosed power consumption, empirically test the SDI-based Γst engineering predic-
tions, and extend Sc to the higher-order (Betti number) formulation.
4
Methods
4.1
Data Provenance and Quality Grading
...
γ = 1.0)
 Rsw = tanh[(σ −1)/2]; σ = (C/Crand)/(L/Lrand)
...
ηI computation: ηI = CST/Pnorm, Pnorm = Psystem/20 W.
4.3
Statistical Analysis
