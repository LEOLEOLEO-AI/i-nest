...
C = global connectivity (LCC fraction); H = hierarchical depth (scale-normalized k-core ratio [Dorogovtsev et al. 2006]); M = resolution-corrected modularity Q' (Louvain Q, corrected for random-graph expectation [Fortunato \& Barthélemy 2007]); $R\_{sw}$ = small-world coefficient (tanh-normalized Watts-Strogatz $\sigma$, Erdős--Rényi baseline [8]). All four components are bounded $\in$ [0,1] by construction under the Unified Cross-Species Computation Protocol (UCCP; see Methods). Critically, $R\_{sw}$ encodes triangular closure through the clustering coefficient \$1_{v}\$ = 2$\cdot$(triangles at v)/(\$1_{v}\$(\$1_{v}\$−1)), capturing pairwise higher-order topology; full simplex-level topology via Betti numbers [54] is discussed in the Extension to Higher-Order Networks section. The geometric mean captures the bottleneck structure: deficiency in any single component drives Sc $\rightarrow$ 0.
...
Intelligence Efficiency \$\eta_{I}\$ extends CST to a sustainability metric:
...
For the BNN cohort, we extracted structural (\$1_{c}\$), temporal (\$1_{c}\$, geometric mean of $\lambda$_eff, \Phi, \Psi, \Theta), and coupling ($\Gamma$st) parameters strictly from authoritative connectomic and electrophysiological literature:
...
- Drosophila Mushroom Body (Scheffer 2020) exhibits highly modular olfactory and learning centers (\$1_{c}\$=0.692 under UCCP), achieving $CST = 1.6692$ (Level III, Creativity, approaching threshold $\phi$=1.618).
...
- Human cerebral cortex (Hagmann 2008) achieves the highest measured complexity (\$1_{c}\$=0.905, \$1_{c}\$=0.872, $\Gamma$st=0.41), peaking at $CST = 3.9198$ (Level V, General Intelligence, threshold $\pi$ $\approx$ 3.1416). The human CST is stable across normalization schemes (V23: 3.9087; UCCP V24: 3.9198; $\Delta$ = +0.28\%), confirming robustness.
...
The binary-digital ceiling: CS\$1_{emergent_max}\$ $\approx$ 0.35 (at $\Gamma$st $\rightarrow$ 0.5, $\alpha$_digital = 0.69)—permanently below L1 = 0.707. No amount of parameter scaling within binary-digital architecture can overcome this exponential ceiling. Importantly, this ceiling is not imposed by CMOS technology; analog CMOS implementations of memristive synapses achieve $\alpha$ $\approx$ 3.5–4.5, lifting the ceiling entirely (see Table 3, Gen1). And crucially, every step toward higher domain-specific CST through scaling demands exponentially greater energy investment: \$\eta_{I}\$ degrades with scale rather than improving.
...
Table 2. ANN architecture innovations mapped to CST dimensions (2017--2025). All systems remain at CS\$1_{emergent}\$ < L1 under binary-digital implementation. CMOS fabrication per se does not impose this constraint---it applies to the binary-logic computational paradigm. References given for all included systems.
...
The sub-linear efficiency law. Independent of CST, empirical measurement now confirms that energy efficiency per unit capability improvement follows a sub-linear (diminishing returns) curve as LLMs scale [arXiv:2501.02156]. CST provides the mechanism: each marginal CS\$1_{func}\$ improvement through parameter scaling requires a proportionally greater energy investment because the binary-digital $\Gamma$st ceiling forces all gains to be achieved through brute-force statistical weight accumulation rather than physical coupling. \$\eta_{I}\$ degrades monotonically with scale, and no architectural refinement within the binary-digital paradigm reverses this trend.
...
$IIL = \$CST_{\text{species}}\$ = ($S$\_c$$ $\cdot$ \$1_{c}\$) $\cdot$ exp($\alpha$ $\cdot$ $\Gamma$st)$
...
Limitations. $\Gamma$st comparability across BNN and ANN measurement modalities is validated within $\pm$0.04 for C. elegans simulations; domain-specific CS\$1_{func}\$ estimates for modern LLMs are theoretical projections without open weight access; the six threshold formal derivations from multiplex percolation theory await companion paper completion. Future work should validate CST in deployed neuromorphic hardware, extend \$\eta_{I}\$ measurements to frontier LLMs with disclosed power consumption, empirically test the SDI-based $\Gamma$st engineering predictions, and extend Sc to the higher-order (Betti number) formulation for systems with documented simplex interactions.
...
- H = min[(\$1_{max}\$ / \$1_{null}\$) / 6.667, 1.0]; \$1_{null}\$ estimated by the analytic Erdős–Rényi approximation \$1_{null}\$ $\approx$ ln(N)/ln(ln(N)) [Dorogovtsev et al. 2006]; anchor: Human HCP (\$1_{max}\$/\$1_{null}\$ = 6.667) $\rightarrow$ H = 1.0


- M = max[(Q − 0.02) / (1 − 0.02), 0.01]; Q = Louvain modularity (100 random restarts, resolution $\gamma$ = 1.0); \$1_{rand}\$ = 0.02 (conservative Erdős–Rényi expectation); floor $\epsilon$ = 0.01 prevents geometric-mean collapse for near-random networks; correction follows Fortunato \& Barthélemy [2007]


- $R\_{sw}$ = tanh[($\sigma$ − 1) / 2]; $\sigma$ = (C/\$1_{rand}\$)/(L/\$1_{rand}\$), Erdős–Rényi baseline (100 realizations); maps $\sigma$ = 1 (random) $\rightarrow$ 0, $\sigma$ = 4.1 (human HCP) $\rightarrow$ 0.914; normalization follows Humphries \& Gurney [2008]
...
Hardware classification: Binary-digital ANN ($\alpha$ = ln(2) = 0.69): MLP, CNN, RNN/LSTM, GNN, Transformer, MoE. Neuromorphic hardware [NMH†] ($\alpha$ = ln(32) = 3.47): SNN Intel Loihi-2. The $\alpha$ value for Loihi-2 is derived from the thermal-noise-limited membrane potential resolution of its CMOS leaky integrate-and-fire implementation: $\sigma$_V = sqrt(kT/\$1_{mem}\$) $\approx$ 0.6 mV against a ~20 mV dynamic range yields SNR $\approx$ 32 $\approx$ 2^5 effective states per timestep [Strong et al. 1998]. This places Loihi-2 at the low end of the biologically observed 3–6 bits/spike range, justifying $\alpha$ = ln(32) = 3.47, distinct from binary-digital $\alpha$ = 0.69. Graded-potential BNN ($\alpha$ = ln(13) = 2.56): E. coli, C. elegans, LTC/NCP. Spiking BNN ($\alpha$ = ln(32)--ln(50) = 3.47--3.91): Zebrafish, Drosophila, Octopus, Mouse, Macaque, Human.
...
4. Figure Legends: All 6 figure descriptions redesigned --- concise ($\leq$100 words each), no text overlap, precise panel descriptions. Figures now: F1=CST decomposition, F2=40-system validation, F3=Triple Lock, F4=Hardware roadmap, F5=\$\eta_{I}\$ comparison, F6=Architectural evolution (2-panel, no overlap).
...
2. Methods/H: Added scale-normalization with Human HCP anchor (\$1_{max}\$/\$1_{null}\$ / 6.667); fixes 14/16 systems that had H > 1.0 in V23 (violating Axiom 1 boundedness).

3. Methods/M: Added \$1_{rand}\$ = 0.02 correction and floor $\epsilon$ = 0.01 to prevent Sc = 0 for near-random networks (E. coli, MLP).
...
6. Discussion: New section "Extension to higher-order networks" with S\$1_{HO}\$ formula and Betti number extension (deferred to companion paper)
...
2. \$\eta_{I}\$ values corrected: human \$\eta_{I}\$ 0.18$\rightarrow$3.67; GPT-4 \$\eta_{I}\$ 4$\times$10⁻⁷$\rightarrow$8.8$\times$10⁻⁶ (per formula \$\eta_{I}\$=CST/\$1_{norm}\$; five-order-of-magnitude gap confirmed at 4.2$\times$10⁵)
...
5. Discussion: \$\eta_{I}\$ value for human brain updated
