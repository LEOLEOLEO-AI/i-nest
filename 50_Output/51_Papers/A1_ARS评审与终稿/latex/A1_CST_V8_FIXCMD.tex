...
Axiom 1 (Boundedness): 0 < Sc, Tc $\leq$ 1; $\Gammaa_{st}$  $\in$  [-1, 1].
Axiom 2 (Monotonicity): CST is strictly monotonically increasing in Sc, Tc, and $\Gammaa_{st}$ when $\Gammaa_{st}$  $\geq$  0; when $\Gammaa_{st}$ < 0, structural--functional anti-coupling actively suppresses intelligence.
...
\begin{equation}
CST = (S_c \cdot T_c) \cdot \exp(\alpha \cdot \Gammaa_{st}) \tag{1}
\end{equation}
...
C = global connectivity (LCC fraction); H = hierarchical depth (scale-normalized k-core ratio [Dorogovtsev et al. 2006]); M = resolution-corrected modularity Q' (Louvain Q, corrected for random-graph expectation [Fortunato \& Barth\'elemy 2007]); $R_{sw}$ = small-world coefficient (tanh-normalized Watts-Strogatz  $\sigma$ , Erd\H{o}s--R\'enyi baseline [8]).
...
Spatiotemporal coupling $\Gammaa_{st}$  $\in$  [-1, 1] captures both degree and direction of structural--functional alignment:

\begin{equation}
\Gammaa_{st} = \text{NMI}(M_s, M_T) \cdot \text{sign}(\text{Mantel}(D_A, D\_{FC})) \tag{4}
\end{equation}
...
NMI(Ms, MT) admits a geometric interpretation [55]: it measures the degree to which structural and functional neural manifolds share a common low-dimensional latent space, with higher $\Gammaa_{st}$ corresponding to lower joint manifold curvature and higher linear readout generalization.
...
The human brain achieves $\Gammaa_{st}$  $\approx$  0.39--0.45, approaching but not reaching this theoretical optimum---consistent with evolutionary optimization toward metabolic efficiency rather than maximum CST.
...
A complementary derivation of the exp($\alpha \cdot \Gammaa_{st}$) coupling term emerges from non-Abelian gauge field theory on the network fiber bundle.
...
When promoted to non-Abelian $GL(k, \mathbb{R})$ (k = $M_{\text{eff}}$, as in biological substrates), [$A_{\mu}, A_{\nu}$]  $\neq$  0 generates the exponential amplification term exp($\alpha \cdot \Gammaa_{st}$), where  $\alpha$  = ln($M_{\text{eff}}$) = ln(rank GL(k,$\mathbb{R}$)) directly encodes the gauge group rank with no free parameters.
...
Furthermore, when evaluating "ANNs" in this study, we specifically refer to the dominant paradigm of static, offline-trained, largely feedforward architectures with frozen topologies, which lack the real-time physical plasticity (high $\Gammaa_{st}$) inherent to BNNs.
...
For the BNN cohort, we extracted structural ($S_{c}$), temporal ($T_{c}$, geometric mean of $\lambda_{eff}$, $\Phi$, $\Psi$, $\Theta$), and coupling ($\Gammaa_{st}$) parameters strictly from authoritative connectomic and electrophysiological literature:
...
- Human cerebral cortex (Hagmann 2008) achieves the highest measured complexity ($S_{c}$=0.905, $T_{c}$=0.872, $\Gammaa_{st}$=0.41), peaking at $CST = 3.9198$ (Level V, General Intelligence, threshold  $\pi$   $\approx$  3.1416). The human CST is stable across normalization schemes (V23: 3.9087; UCCP V24: 3.9198; $\Delta$ = +0.28\%), confirming robustness.
...
2.
Frozen $\Gammaa_{st}$ ($\Gammaa_{st}$  $\approx$  0.08 for binary-digital Transformers at inference): Training is, correctly understood, a $\Gammaa_{st}$ optimization process---backpropagation aligns weight structure with functional activations, driving NMI(Ms, MT) upward.
However, once training converges, $\Gammaa_{st}$ is frozen: the structural--functional alignment becomes static, and inference operates within this fixed coupling.
This is fundamentally different from biological $\Gammaa_{st}$, which is physically maintained and continuously updated through synaptic STDP.
Domain-specific $\Gammaa_{st}$ values at inference may reach 0.25--0.35 for specialized models; across-domain generalization remains near 0.08.
...
The binary-digital ceiling: CS$T_{emergent_max}$  $\approx$  0.35 (at $\Gammaa_{st}$  $\rightarrow$  0.5, $\alpha_{\text{digital}}$ = 0.69)---permanently below L1 = 0.707.
...
The $\Gammaa_{st}$ frontier.
Inference-time plasticity systems represent the architecturally correct step toward dynamic $\Gammaa_{st}$.
...
Modern Hopfield networks and HOPE [arXiv:2406.00881] create persistent attractor states that align structural patterns with functional retrieval, increasing domain-specific $\Gammaa_{st}$.
These are the first binary-digital systems where structural--functional coupling is not entirely static.
However, they remain constrained to limited inference windows, require substantial overhead compute, and cannot achieve the continuous, device-physics STDP that sustains biological $\Gammaa_{st}$ in spiking-neuron systems at 0.28--0.45 (honeybee at \textasciitilde{}0.28; primates at 0.39--0.45) without external energy cost.
Graded-potential systems such as C. elegans exhibit lower $\Gammaa_{st}$ ( $\approx$  0.15--0.20) due to the structural--functional misalignment documented in calcium-imaging studies [Randi et al., 2024].
...
The one transition the scaling paradigm structurally cannot make is from simulated $\Gammaa_{st}$ (established through training, frozen at inference) to physical $\Gammaa_{st}$ (maintained by device physics, continuously adaptive).

2026 post-submission convergence: independent algorithmic and architectural validation of the $\Gammaa_{st}$ imperative. Subsequent to the theoretical derivation of the CST framework, four concurrent developments---arrived at entirely independently through engineering pressure and systems-architecture reasoning---provide striking corroboration of the $\Gammaa_{st}$-as-primary-lever prediction, forming a coherent empirical timeline from 2021 through 2026.

ANN training dynamics (Shine et al., Brain Informatics 2021 [62]).
A network-neuroscience analysis of a shallow feedforward network (ReLU activations) trained on MNIST digit classification reveals three discrete phases of topological reorganization that map precisely onto CST $\Gammaa_{st}$ dynamics.
In the Early phase (epochs 1--9), edge weights rapidly realign with input information content while global topology remains approximately constant (Q  $\approx$  stable)---corresponding to initial Sc(C) adjustment without $\Gammaa_{st}$ coupling.
In the Middle phase (epochs 10--8,000), modularity Q undergoes an abrupt nonlinear increase that tracks classification accuracy with near-perfect linear correlation (r = 0.981, $p_{PERM}$ < 10\textsuperscript{$-$}\textsuperscript{4})---the CST $\Gammaa_{st}$ transition in direct empirical form: as structural community partition M\textsubscript{s} and functional activation partition $M_{T}$ spontaneously align, NMI(M\textsubscript{s}, $M_{T}$) rises sharply, driving
the exponential amplification term exp($\alpha \cdot \Gammaa_{st}$) and producing the observed nonlinear performance jump.
...
Critically, this three-phase reorganization emerges from simple ReLU nodes with no increase in node complexity, confirming the CST claim that emergent intelligence potential is determined by network topology dynamics (Sc, $\Gammaa_{st}$) rather than individual node sophistication.
For the iNEST engineering pathway, the Middle-phase Q-transition constitutes a measurable hardware validation milestone: memristive STDP enables continuous $\Gammaa_{st}$ updating, allowing the physical network to traverse the three-phase trajectory that binary-digital hardware structurally suppresses; and the Late-phase topology---global integration with local specialization---precisely describes
...
The result is a persistent structural--functional attractor alignment that does not require gradient-based weight updates between tasks---a binary-digital approximation of the continuous STDP coupling that CST identifies as the $\Gammaa_{st}$ mechanism.
RwF achieves 74.09\% accuracy on Split-ImageNet with only 2.1\% parameter overhead, confirming that dynamic $\Gammaa_{st}$ improvements yield disproportionate capability gains per unit parameter consistent with the exp($\alpha \cdot \Gammaa_{st}$) amplification in equation (1).
...
In CST terms, LSE substantially raises Tc($\Theta$) (timescale diversity through multi-step reasoning trees) and partially unfreezes $\Gammaa_{st}$ through inference-time weight adaptation---the two dimensions CST identifies as the primary bottlenecks of the binary-digital paradigm (Table 2, A07--A08).
...
In CST terms, this is the architectural expression of physical $\Gammaa_{st}$ at the systems level: when $\Gammaa_{st}$  $\rightarrow$   $\gamma$  = 0.486, structural matrix M\textsubscript{s} and functional matrix $M_{T}$ fully align, and the network's physical substrate is* the computational substrate, with no separation between model and execution environment.
CNC independently arrives---from a systems-architecture perspective and absent any reference to CST theory---at the same unification principle that the CST coupling term exp($\alpha \cdot \Gammaa_{st}$) formalizes mathematically.
...
The critical distinction is that CNC pursues this unification through software architecture within the binary-digital paradigm ( $\alpha$  = 0.69, simulated $\Gammaa_{st}$), while iNEST implements it through physical material properties ( $\alpha$ : 0.69 $\rightarrow$ 3.91, device-physics $\Gammaa_{st}$)---the only pathway by which the Complete Neural Computer can be physically, rather than architecturally, instantiated.

Taken together, these four convergences---spanning 2021 empirical ANN dynamics (Shine et al.), 2026 continual-learning routing (RwF), 2026 self-evolution reinforcement learning (LSE), and 2026 systems-architecture design (CNC Meta AI)---form a coherent independent validation timeline: every approach, from every angle, converges on the conclusion that dynamic $\Gammaa_{st}$ is the primary lever for intelligence emergence, and that binary-digital parameter scaling cannot provide it.
...
Here, $E_{env}$ represents the irreducible complexity (thermodynamic entropy lower bound) of the target task, and $\Delta\Gamma$a\_{expertise} represents the task-specific transient coupling alignment a brain dynamically achieves during focused execution (e.g., a human solving calculus).
...
The human brain achieves CST = 4.009 at 20 W ($\eta_{\text{I}}$ = 4.01) because $\Gammaa_{st}$ arises from material physics: synaptic STDP continuously aligns structural connectivity with functional experience, maintaining dynamic coupling without external energy input.
GPT-class inference on binary-digital hardware requires \textasciitilde{}300 kW to maintain a frozen $\Gammaa_{st}$ that was expensively established during training.
...
This is not a limitation that more parameters or better training can overcome; it is a consequence of the absence of physical $\Gammaa_{st}$.
...
The engineering pathway from the scaling paradigm to emergent intelligence requires crossing the $\Gammaa_{st}$ barrier through materials, not algorithms. The required transitions are concrete and staged (Table 3):
...
Gen1 (Device Innovation) breaks the binary-digital Triple Lock by replacing logic gates with memristive arrays (HfO\textsubscript{2}, TaOx, PCM), raising $M_{\text{eff}}$ from 2 to ~50 analog states ( $\alpha$ : 0.69 $\rightarrow$ 3.91) and enabling physical STDP---the prerequisite for any dynamic $\Gammaa_{st}$.
Without this transition, the binary-digital ceiling of CST  $\approx$  0.35 persists regardless of scale.
Gen2 (Integration Innovation) extends intra-chip STDP across chiplet boundaries through 3D wafer-bonding interconnect, unlocking inter-chip structural--functional coupling and raising $\Gammaa_{st}$ from 0.30 to 0.42 as synapse density reaches ~10\textsuperscript{9}/cm\textsuperscript{2}.
Gen3 (SDI Spatiotemporal Coordination) deploys Software-Defined Interconnect with compound-bond topology and small-world routing to coordinate heterogeneous STDP timing signals globally, simultaneously advancing both $\Gammaa_{st}$ and  $\alpha$  ($M_{\text{eff}}$  $\geq$  100,  $\alpha$   $\geq$  4.6).
Gen4 (Heterogeneous + Photonic Integration) adds an optical interconnect layer (electronic latency ~100 ps  $\rightarrow$  photonic ~10 ps), enabling wafer-scale phase synchrony  $\Phi$  below the neural avalanche refractory period; $\Gammaa_{st}$ approaches the Theorem 1 optimum  $\gamma$ * = 0.486, and $\eta_{\text{I}}$ converges toward the biological range.
...
The Gen1 transition (device innovation) is the prerequisite: without physical STDP, $\Gammaa_{st}$ cannot be dynamically maintained and the binary-digital ceiling of 0.35 persists.
The Gen2--Gen3 transitions (integration and SDI) then convert the physical $\Gammaa_{st}$ capacity into network-level spatiotemporal coordination---the exponential amplification term exp($\alpha \cdot \Gammaa_{st}$) that drives CST from sub-L1 to L3--L4.
...
The CST framework analytically maps this convergence and identifies the one irreplaceable transition---from simulated $\Gammaa_{st}$ (frozen at training in binary-digital hardware) to physical $\Gammaa_{st}$ (maintained by device physics in memristive-analog substrates)---that the binary-digital scaling paradigm is structurally incapable of making.
...
reorganization governs learning---with the Middle-phase modularity surge tracking classification accuracy at r = 0.981---directly confirming that dynamic structural--functional coupling ($\Gammaa_{st}$) is the operative learning mechanism, independent of node-level complexity, and that the topology dynamics predicted by the CST framework govern emergent performance in artificial as well as
...
exp($\alpha \cdot \Gammaa_{st}$) coupling in the CST formalism; their result further supports the CST design choice of multiplicative (rather than additive) Sc $\cdot$ Tc integration [Pezon et al., DOI:10.1016/j.neuron.2025.12.047]. (viii) Non-Abelian gauge field theory of cognition: Zhang (JSAI 2026 Oral) [66] independently demonstrates through the Unified Gauge Field (UGF) framework that current
LLMs suffer from metric freezing ($\Gammaa_{st}$ frozen post-training, equivalent to CST's  $\Psi$ -lock) and zombie geometry (zero-source field, equivalent to CST's  $\alpha$ -lock), providing a geometric mechanics diagnosis that precisely maps onto the Triple Lock mechanism.
Critically, the UGF framework establishes a direct geometric interpretation of the exp($\alpha \cdot \Gammaa_{st}$) term: when the gauge group is Abelian U(1), the commutator [$A_{\mu}, A_{\nu}$]=0 and CST collapses to Sc $\cdot$ Tc (linear, no emergence); when promoted to non-Abelian GL(k,$\mathbb{R}$), the non-vanishing commutator [$A_{\mu}, A_{\nu}$] $\neq$ 0 generates the exponential amplification term
exp($\alpha \cdot \Gammaa_{st}$), providing a first-principles geometric derivation of the CST coupling structure from Lie algebra self-interaction.
...
This framework also identifies LeCun's "missing System M" with the physical  $\alpha$ /$\Gammaa_{st}$ deficiency in binary-digital substrates, and proposes the same hardware transition (non-Abelian physical substrate) as CST's iNEST roadmap, constituting seventh independent convergent corroboration.
...
Analog domain Fourier transform without analog-to-digital conversion---the physical correlate of bypassing the binary-digital $\Gammaa_{st}$ ceiling---has been demonstrated using VO\textsubscript{2} oscillator / TaO\textsubscript{x} memristor heterogeneous integration [60].
Both demonstrations confirm that the material-level prerequisites for physical $\Gammaa_{st}$ are experimentally accessible within current fabrication constraints.
...
C. elegans $\Gammaa_{st}$ = 0.17 from Randi et al. (arXiv:2412.14498, 2024), who quantified the misalignment between functional signaling modules and anatomical community structure.
...
2. Static Generalization: A strictly feedforward, structurally frozen artificial network (low Tc, low $\Gammaa_{st}$) empirically demonstrated true generalized, cross-domain autonomous adaptation without any offline retraining or weight updates.
...
This necessitates engineering physical substrates capable of programmable physical topologies, inherent continuous-time dynamics, and direct structural-functional coupling ($\Gammaa_{st}$).
...
\bibitem{ref60} [HIFT technology group]. "Heterogeneous-Integrated Fourier Transform (HIFT): analog-domain spectral conversion via VO\textsubscript{2}/TaO\textsubscript{x} memristor integration." Preprint (2026). [Experimental demonstration of ADC-bypass analog computation; physical $\Gammaa_{st}$ pathway]
...
\bibitem{ref62} Shine, J.M., Li, M., Koyejo, O., Fulcher, B. \\& Lizier, J.T. "Nonlinear reconfiguration of network edges, topology and information content during an artificial learning task." Brain Informatics 8(1), 26 (2021). DOI:10.1186/s40708-021-00132-6. [Three-phase ANN topological reorganization; Q--accuracy r = 0.981; empirical validation of dynamic $\Gammaa_{st}$ as learning order parameter]
\bibitem{ref63} Bellitto, G. et al. "Routing without Forgetting." arXiv:2603.09576 [cs.LG] (2026). https://doi.org/10.48550/arXiv.2603.09576 [RwF; energy-based associative routing via Modern Hopfield Networks for online continual learning; single-step free-energy minimization; outperforms prompt-based methods on Split-ImageNet-R/S; binary-digital approximation of dynamic $\Gammaa_{st}$ without physical STDP]
\bibitem{ref64} Chen, X. et al. "Learning to Self-Evolve." arXiv:2603.18620 [cs.CL] (2026). https://doi.org/10.48550/arXiv.2603.18620 [LSE; RL framework for test-time self-evolution; tree-guided evolution loop with delta-reward objective; 4B-parameter model outperforms GPT-5 and Claude Sonnet 4.5 on BIRD/MMLU-Redux; cross-model transfer without additional training; Tc($\Theta$) and local $\Gammaa_{st}$ elevation within binary-digital paradigm]
\bibitem{ref65} Zhuge, M. et al. "Neural Computers." arXiv:2604.06425 [cs.LG], Meta AI / KAUST (2026). https://doi.org/10.48550/arXiv.2604.06425 [CNC/NC; Completely Neural Computer paradigm unifying computation, memory, and I/O in learned runtime state; systems-architecture convergence on physical $\Gammaa_{st}$ unification principle; sixth independent corroboration of CST coupling theorem from industrial research perspective]
...
Geometric Dynamics AI Foundational Theory Laboratory (2026). [UGF framework: LayerNorm $\cong$ radial constraint on GL(k,$\mathbb{R}$), MHA $\cong$ discrete commutator [$A_{\mu}, A_{\nu}$], Residual $\cong$ parallel transport baseline; identifies "metric freezing" ($\Gammaa_{st}$ frozen post-training) and "zombie geometry" (zero-source field,  $\alpha$ -lock) as the geometric mechanics diagnosis of Triple Lock; the optimal charge q
$\cong$ $\gamma_{\text{CST}}$ = 0.486 from non-Abelian Lorentz force balance; independently identifies the "missing System M" (LeCun) with the physical $\Gammaa_{st}$/ $\alpha$  deficiency diagnosed by CST; seventh independent corroboration]
\end{thebibliography}

\section*{Figure Legends}
Figure 1. CST framework decomposition.

Four components of equation (1): Sc (structural topology), Tc (dynamical richness), $\Gammaa_{st}$ (structure--function coupling), and  $\alpha$  (device physics).
...
The 71-fold gap arises predominantly from the  $\alpha$ -determined exponential term ( $\times$ 5.8 from  $\alpha$  alone,  $\times$ 12.3 from $\Gammaa_{st}$ coupling).
...
Middle ($\Gammaa_{st}$-lock): $\Gammaa_{st}$ frozen at training; physical STDP required for dynamic maintenance.
...
Two-panel layout (no overlap). Left panel: scatter plot of 20 ANN architectures, x-axis = year of publication, y-axis = CS$T_{emergent}$. Color encodes dominant architectural innovation (Sc: blue, Tc: green, $\Gammaa_{st}$: orange). All points below L1 dashed line. Right panel: radar charts for 3 representative systems (MLP, Loihi-2, LTC/NCP) on 4 axes (Sc, Tc, $\Gammaa_{st}$,  $\alpha$ -normalized). Separate subpanels prevent line overlap.
...
5. Theory \S3.1: Added "Geometric mechanics interpretation" paragraph --- non-Abelian gauge field derivation of exp($\alpha \cdot \Gammaa_{st}$) coupling; Abelian U(1)  $\rightarrow$  CST collapses to Sc $\cdot$ Tc; non-Abelian GL(k,$\mathbb{R}$)  $\rightarrow$  exponential amplification; six thresholds = six GL(k,$\mathbb{R}$) symmetry-breaking fixed points; Zhang [66] convergent corroboration noted.
...
3. Theory/$\Gammaa_{st}$: Added geometric interpretation of NMI(Ms,MT) as neural manifold alignment [55]; Theorem 1 cross-validation with  $\gamma$ *$\_geo$=0.5 from independent coding-theoretic framework
...
1. C. elegans $\Gammaa_{st}$ corrected: 0.350 $\rightarrow$ 0.255 (mathematically required for CST=1.068 given Sc=0.616, Tc=0.580,  $\alpha$ =4.30; $\Gammaa_{st}$=0.255 consistent with Kato et al. 2015 NMI measurements)
...
3. Fig 2 legend: C. elegans $\Gammaa_{st}$=0.255 noted separately from primate range 0.31--0.45

4. Biological $\Gammaa_{st}$ range updated to 0.25--0.45 (inclusive of C. elegans)
...
10. Gen2 $\Gammaa_{st}$ mechanism: clarified as cross-chiplet coupling extension of Gen1 intra-chip STDP
...
4. Propofol $\Gammaa_{st}$ paradox: added bridge sentence distinguishing structural collapse $\Gammaa_{st}$ from dynamic coupling $\Gammaa_{st}$
