...
C = global connectivity (LCC fraction); H = hierarchical depth (scale-normalized k-core ratio [Dorogovtsev et al. 2006]); M = resolution-corrected modularity Q' (Louvain Q, corrected for random-graph expectation [Fortunato \& Barthélemy 2007]); $R\_{sw}$ = small-world coefficient (tanh-normalized Watts-Strogatz $\sigma$, Erdős--Rényi baseline [8]). All four components are bounded $\in$ [0,1] by construction under the Unified Cross-Species Computation Protocol (UCCP; see Methods). Critically, $R\_{sw}$ encodes triangular closure through the clustering coefficient $C_{v}$ = 2$\cdot$(triangles at v)/($k_{v}$($k_{v}$−1)), capturing pairwise higher-order topology; full simplex-level topology via Betti numbers [54] is discussed in the Extension to Higher-Order Networks section. The geometric mean captures the bottleneck structure: deficiency in any single component drives Sc $\rightarrow$ 0.
...
where $P_{norm}$ = P / 20W (normalizing to the human brain's resting power). This separates the question of what level of intelligence a system achieves from at what energetic cost. Human brain: $\eta$_I = 3.91 (CST = 3.909, $P_{norm}$ = 1; $\alpha$_cortical = ln(50) $\approx$ 3.91, $M_{\text{eff}}$ = 50 as conservative estimate following Rieke et al. [Spikes, 1996]). GPT-4 class inference (~300 kW estimated system-level infrastructure power [see Methods]): $\eta$_I $\approx$ 8.8$\times$10⁻⁶. The six-order-of-magnitude gap is not an engineering problem; it is a thermodynamic signature of the difference between emergent and simulated intelligence.
...
For the BNN cohort, we extracted structural ($S_{c}$), temporal ($T_{c}$, geometric mean of $\lambda_{eff}$, \Phi, \Psi, \Theta), and coupling ($\Gamma$st) parameters strictly from authoritative connectomic and electrophysiological literature:
...
- Human cerebral cortex (Hagmann 2008) achieves the highest measured complexity ($S_{c}$=0.905, $T_{c}$=0.872, $\Gamma$st=0.41), peaking at $CST = 3.9198$ (Level V, General Intelligence, threshold $\pi$ $\approx$ 3.1416). The human CST is stable across normalization schemes (V23: 3.9087; UCCP V24: 3.9198; $\Delta$ = +0.28\%), confirming robustness.
...
Tc improvements. Spiking Neural Networks (Intel Loihi-2, SpiNNaker2) introduce genuine neural avalanche dynamics, raising $\lambda_{eff}$ toward the critical branching ratio ($\lambda_{eff}$ $\rightarrow$ 1) while increasing $\alpha$ through higher $M_{\text{eff}}$ of analog spike-timing states. Liquid Neural Networks (LNN/NCP [Nature Machine Intelligence 2022]) exploit continuous-time ODE dynamics with adaptive time constants, directly improving functional connectivity variability $\Psi$ and timescale diversity $\Theta$—the two Tc components most severely suppressed by frozen Transformer inference. Selective SSMs (Mamba [33], RWKV) restore temporal criticality by reintroducing selective recurrence, increasing $\lambda_{eff}$ relative to attention-only baselines. Extended reasoning systems (OpenAI o1, DeepSeek-R1 [arXiv:2501.12948]) extend $\Theta$ by creating explicit multi-step temporal structure---hundreds of reasoning steps creating a hierarchy of timescales absent in single-pass inference.
...
$IIL = $CST_{\text{species}}$ = ($S_{c}$ $\cdot$ $T_{c}$) $\cdot$ exp($\alpha$ $\cdot$ $\Gamma$st)$
...
- H = min[($k_{max}$ / $k_{null}$) / 6.667, 1.0]; $k_{null}$ estimated by the analytic Erdős–Rényi approximation $k_{null}$ $\approx$ ln(N)/ln(ln(N)) [Dorogovtsev et al. 2006]; anchor: Human HCP ($k_{max}$/$k_{null}$ = 6.667) $\rightarrow$ H = 1.0


- M = max[(Q − 0.02) / (1 − 0.02), 0.01]; Q = Louvain modularity (100 random restarts, resolution $\gamma$ = 1.0); $Q_{rand}$ = 0.02 (conservative Erdős–Rényi expectation); floor $\epsilon$ = 0.01 prevents geometric-mean collapse for near-random networks; correction follows Fortunato \& Barthélemy [2007]


- $R\_{sw}$ = tanh[($\sigma$ − 1) / 2]; $\sigma$ = (C/$C_{rand}$)/(L/$L_{rand}$), Erdős–Rényi baseline (100 realizations); maps $\sigma$ = 1 (random) $\rightarrow$ 0, $\sigma$ = 4.1 (human HCP) $\rightarrow$ 0.914; normalization follows Humphries \& Gurney [2008]


Temporal complexity: Tc = ($\lambda_{eff}$ $\cdot$ $\Phi$ $\cdot$ $\Psi$ $\cdot$ $\Theta$)^(1/4), where all four components are independently bounded [0,1]. For BNN: $\lambda_{eff}$ = avalanche branching ratio [Beggs \& Plenz 2003]; $\Phi$ = mean pairwise PLV across $\theta$/$\alpha$/$\gamma$ bands; $\Psi$ = std(100 sliding-window FC matrices)/mean|FC|; $\Theta$ = Shannon entropy of intrinsic timescale distribution (10 log-spaced bins), normalized by log₂(10) [Murray et al. 2014]. For ANN: $\lambda_{eff}$ = activation propagation branching ratio (layer-wise active-unit ratio); $\Phi$ = mean pairwise CKA across layer pairs [Raghu et al. 2021]; $\Psi$ = std(100-batch activation correlation matrices)/mean|C|; $\Theta$ = Shannon entropy of layer autocorrelation decay constants. Cross-modal calibration for $\Phi$ (PLV↔CKA): Randi et al. 2024 + Raghu et al. 2021, validated within $\pm$0.04.


Hardware classification: Binary-digital ANN ($\alpha$ = ln(2) = 0.69): MLP, CNN, RNN/LSTM, GNN, Transformer, MoE. Neuromorphic hardware [NMH†] ($\alpha$ = ln(32) = 3.47): SNN Intel Loihi-2. The $\alpha$ value for Loihi-2 is derived from the thermal-noise-limited membrane potential resolution of its CMOS leaky integrate-and-fire implementation: $\sigma$_V = sqrt(kT/$C_{mem}$) $\approx$ 0.6 mV against a ~20 mV dynamic range yields SNR $\approx$ 32 $\approx$ 2^5 effective states per timestep [Strong et al. 1998]. This places Loihi-2 at the low end of the biologically observed 3–6 bits/spike range, justifying $\alpha$ = ln(32) = 3.47, distinct from binary-digital $\alpha$ = 0.69. Graded-potential BNN ($\alpha$ = ln(13) = 2.56): E. coli, C. elegans, LTC/NCP. Spiking BNN ($\alpha$ = ln(32)--ln(50) = 3.47--3.91): Zebrafish, Drosophila, Octopus, Mouse, Macaque, Human.
...
Two-panel layout (no overlap). Left panel: scatter plot of 20 ANN architectures, x-axis = year of publication, y-axis = CS$T_{emergent}$. Color encodes dominant architectural innovation (Sc: blue, Tc: green, $\Gamma$st: orange). All points below L1 dashed line. Right panel: radar charts for 3 representative systems (MLP, Loihi-2, LTC/NCP) on 4 axes (Sc, Tc, $\Gamma$st, $\alpha$-normalized). Separate subpanels prevent line overlap.
...
2. $\eta$_I values corrected: human $\eta$_I 0.18$\rightarrow$3.67; GPT-4 $\eta$_I 4$\times$10⁻⁷$\rightarrow$8.8$\times$10⁻⁶ (per formula $\eta$_I=CST/$P_{norm}$; five-order-of-magnitude gap confirmed at 4.2$\times$10⁵)
