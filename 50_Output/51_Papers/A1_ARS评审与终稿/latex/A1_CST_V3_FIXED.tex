...
\begin{equation}
CST = (S_c $\cdot$ T_c) $\cdot$ \exp($\alpha$ $\cdot$ $\Gamma$_{st}) \tag{1}
\end{equation}
...
\begin{equation}
T_c = ($\lambda$_eff $\cdot$ $\Phi$ $\cdot$ $\Psi$ $\cdot$ $\Theta$)^{1/4} \tag{3}
\end{equation}
...
\begin{equation}
$\Gamma$_{st} = \text{NMI}(M_s, M_T) $\cdot$ \text{sign}(\text{Mantel}(D_A, D\_{FC})) \tag{4}
\end{equation}
...
\begin{equation}
S_c^{\text{HO}} = (X_1 $\cdot$ X_2 $\cdot$ X_3 $\cdot$ X_4 $\cdot$ $\beta$_1^{\text{norm}})^{1/5}
\end{equation}
