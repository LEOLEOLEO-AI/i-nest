...
C = global connectivity (LCC fraction); H = hierarchical depth (scale-normalized k-core ratio [Dorogovtsev et al. 2006]); M = resolution-corrected modularity Q' (Louvain Q, corrected for random-graph expectation [Fortunato \& Barth\'{e}lemy 2007]); $R\_{sw}$ = small-world coefficient (tanh-normalized Watts-Strogatz $\sigma$, Erd\H{o}s--R\'{e}nyi baseline [8]). All four components are bounded  \\in  [0,1] by construction under the Unified Cross-Species Computation Protocol (UCCP; see Methods). Critically, $R\_{sw}$ encodes triangular closure through the clustering coefficient $C_{\text{v}}$ = 2 \\cdot (triangles at v)/($k_{\text{v}}$($k_{\text{v}}$--1)), capturing pairwise higher-order topology; full simplex-level topology via Betti numbers [54] is discussed in the Extension to Higher-Order Networks section. The geometric mean captures the bottleneck structure: deficiency in any single component drives Sc $\rightarrow$ 0.
...
$\lambda_{\text{eff}}$ is the neural avalanche branching ratio (criticality proxy [6]); $\Phi$ is inter-regional phase synchrony; $\Psi$ is functional connectivity temporal variability; $\Theta$ is timescale diversity (Shannon entropy of intrinsic timescale distribution [10]).
...
where $P_{\text{norm}}$ = P / 20W (normalizing to the human brain's resting power). This separates the question of what level of intelligence a system achieves from at what energetic cost. Human brain: $\eta_{\text{I}}$ = 3.91 (CST = 3.909, $P_{\text{norm}}$ = 1; $\alpha_{\text{cortical}}$ = ln(50)  \\approx  3.91, $M_{\text{eff}}$ = 50 as conservative estimate following Rieke et al. [Spikes, 1996]). GPT-4 class inference (~300 kW estimated system-level infrastructure power [see Methods]): $\eta_{\text{I}}$  \\approx  8.8 \\times 10$^{-6}$. The six-order-of-magnitude gap is not an engineering problem; it is a thermodynamic signature of the difference between emergent and simulated intelligence.
...
Geometric mechanics interpretation. [Detailed gauge-theoretic derivation deferred to companion paper [66]; see Supplementary for full treatment.]
...
For the BNN cohort, we extracted structural ($S_{\text{c}}$), temporal ($T_{\text{c}}$, geometric mean of $\lambda_{\text{eff}}$, \Phi, \Psi, \Theta), and coupling ($\Gamma_{st}$) parameters strictly from authoritative connectomic and electrophysiological literature:
...
- Drosophila Mushroom Body (Scheffer 2020) exhibits highly modular olfactory and learning centers ($S_{\text{c}}$=0.692 under UCCP), achieving $CST = 1.6692$ (Level III, Creativity, approaching threshold $\phi$=1.618).
...
- Human cerebral cortex (Hagmann 2008) achieves the highest measured complexity ($S_{\text{c}}$=0.905, $T_{\text{c}}$=0.872, $\Gamma_{st}$=0.41), peaking at $CST = 3.9198$ (Level V, General Intelligence, threshold $\pi$  \\approx  3.1416). The human CST is stable across normalization schemes (V23: 3.9087; UCCP V24: 3.9198; $\Delta$ = +0.28\%), confirming robustness.
...
The binary-digital ceiling: CS$T_{\text{emergent}}_{\text{max}}$  \\approx  0.35 (at $\Gamma_{st}$ $\rightarrow$ 0.5, $\alpha_{\text{digital}}$ = 0.69)---permanently below L1 = 0.707. No amount of parameter scaling within binary-digital architecture can overcome this exponential ceiling. Importantly, this ceiling is not imposed by CMOS technology; analog CMOS implementations of memristive synapses achieve $\alpha$  \\approx  3.5--4.5, lifting the ceiling entirely (see Table 3, Gen1). And crucially, every step toward higher domain-specific CST through scaling demands exponentially greater energy investment: $\eta_{\text{I}}$ degrades with scale rather than improving.
...
Table 2. ANN architecture innovations mapped to CST dimensions (2017--2025). All systems remain at CS$T_{\text{emergent}}$ < L1 under binary-digital implementation. CMOS fabrication per se does not impose this constraint---it applies to the binary-logic computational paradigm. References given for all included systems.
...
Tc improvements. Spiking Neural Networks (Intel Loihi-2, SpiNNaker2) introduce genuine neural avalanche dynamics, raising $\lambda_{\text{eff}}$ toward the critical branching ratio ($\lambda_{\text{eff}}$ $\rightarrow$ 1) while increasing $\alpha$ through higher $M_{\text{eff}}$ of analog spike-timing states. Liquid Neural Networks (LNN/NCP [Nature Machine Intelligence 2022]) exploit continuous-time ODE dynamics with adaptive time constants, directly improving functional connectivity variability $\Psi$ and timescale diversity $\Theta$---the two Tc components most severely suppressed by frozen Transformer inference. Selective SSMs (Mamba [33], RWKV) restore temporal criticality by reintroducing selective recurrence, increasing $\lambda_{\text{eff}}$ relative to attention-only baselines. Extended reasoning systems (OpenAI o1, DeepSeek-R1 [arXiv:2501.12948]) extend $\Theta$ by creating explicit multi-step temporal structure---hundreds of reasoning steps creating a hierarchy of timescales absent in single-pass inference.
...
$IIL = CS$T_{species}$ = ($S_{c}$  \\cdot  $T_{\text{c}}$)  \\cdot  exp($\alpha$  \\cdot  $\Gamma_{st}$)$
...
Data sources. C. elegans: Varshney et al. [11], 279 neurons, 2,990 synapses (wormatlas.org). Mouse: Oh et al. [24], Allen Brain Connectivity Atlas. Human: Van Essen et al. [25], Human Connectome Project. Branching ratio: Beggs \& Plenz [6]. SC--FC coupling: Arnatkeviciute et al. [26]; Honey et al. [41]. C. elegans functional dynamics (Tc components): Kato et al. [34] (whole-brain calcium imaging; $\Psi$ and $\Theta$ estimation); Gordus et al. (2015) Cell 161, 307--320 (circuit-level dynamics; $\lambda_{\text{eff}}$ estimation). C. elegans $\Gamma_{st}$ = 0.17 from Randi et al. (arXiv:2412.14498, 2024), who quantified the misalignment between functional signaling modules and anatomical community structure. C. elegans $\alpha$ = $\alpha_{\text{graded}}$ = ln(13)  \\approx  2.56, derived from graded-potential dynamic range (\textasciitilde{}40 mV) and voltage resolution (\textasciitilde{}3 mV) following Liu et al. (PNAS 2009) and Lockery (Curr. Biol. 2009); HH-model $\alpha$ is inapplicable to predominantly non-spiking neurons. Human $\alpha$ = $\alpha_{\text{cortical}}$ = ln(50)  \\approx  3.91, conservative estimate from Rieke et al. (Spikes, 1996) and consistent with Strong et al. (Science 1998) lower bound. ANN: PyTorch v2.x open-weight implementations.
...
- H = min[($k_{\text{max}}$ / $k_{\text{null}}$) / 6.667, 1.0]; $k_{\text{null}}$ estimated by the analytic Erd\H{o}s--R\'{e}nyi approximation $k_{\text{null}}$  \\approx  ln(N)/ln(ln(N)) [Dorogovtsev et al. 2006]; anchor: Human HCP ($k_{\text{max}}$/$k_{\text{null}}$ = 6.667) $\rightarrow$ H = 1.0


- M = max[(Q -- 0.02) / (1 -- 0.02), 0.01]; Q = Louvain modularity (100 random restarts, resolution $\gamma$ = 1.0); $Q_{\text{rand}}$ = 0.02 (conservative Erd\H{o}s--R\'{e}nyi expectation); floor $\epsilon$ = 0.01 prevents geometric-mean collapse for near-random networks; correction follows Fortunato \& Barth\'{e}lemy [2007]


- $R\_{sw}$ = tanh[($\sigma$ -- 1) / 2]; $\sigma$ = (C/$C_{\text{rand}}$)/(L/$L_{\text{rand}}$), Erd\H{o}s--R\'{e}nyi baseline (100 realizations); maps $\sigma$ = 1 (random) $\rightarrow$ 0, $\sigma$ = 4.1 (human HCP) $\rightarrow$ 0.914; normalization follows Humphries \& Gurney [2008]


Temporal complexity: Tc = ($\lambda_{\text{eff}}$  \\cdot  $\Phi$  \\cdot  $\Psi$  \\cdot  $\Theta$)^(1/4), where all four components are independently bounded [0,1]. For BNN: $\lambda_{\text{eff}}$ = avalanche branching ratio [Beggs \& Plenz 2003]; $\Phi$ = mean pairwise PLV across $\theta$/$\alpha$/$\gamma$ bands; $\Psi$ = std(100 sliding-window FC matrices)/mean|FC|; $\Theta$ = Shannon entropy of intrinsic timescale distribution (10 log-spaced bins), normalized by log$_2$(10) [Murray et al. 2014]. For ANN: $\lambda_{\text{eff}}$ = activation propagation branching ratio (layer-wise active-unit ratio); $\Phi$ = mean pairwise CKA across layer pairs [Raghu et al. 2021]; $\Psi$ = std(100-batch activation correlation matrices)/mean|C|; $\Theta$ = Shannon entropy of layer autocorrelation decay constants. Cross-modal calibration for $\Phi$ (PLV$\leftrightarrow$CKA): Randi et al. 2024 + Raghu et al. 2021, validated within  \\pm 0.04.


Hardware classification: Binary-digital ANN ($\alpha$ = ln(2) = 0.69): MLP, CNN, RNN/LSTM, GNN, Transformer, MoE. Neuromorphic hardware [NMH\dag] ($\alpha$ = ln(32) = 3.47): SNN Intel Loihi-2. The $\alpha$ value for Loihi-2 is derived from the thermal-noise-limited membrane potential resolution of its CMOS leaky integrate-and-fire implementation: $\sigma_{\text{V}}$ = sqrt(kT/$C_{\text{mem}}$)  \\approx  0.6 mV against a ~20 mV dynamic range yields SNR  \\approx  32  \\approx  2^5 effective states per timestep [Strong et al. 1998]. This places Loihi-2 at the low end of the biologically observed 3--6 bits/spike range, justifying $\alpha$ = ln(32) = 3.47, distinct from binary-digital $\alpha$ = 0.69. Graded-potential BNN ($\alpha$ = ln(13) = 2.56): E. coli, C. elegans, LTC/NCP. Spiking BNN ($\alpha$ = ln(32)--ln(50) = 3.47--3.91): Zebrafish, Drosophila, Octopus, Mouse, Macaque, Human.
...
\bibitem{ref60} [HIFT technology group]. "Heterogeneous-Integrated Fourier Transform (HIFT): analog-domain spectral conversion via VO$_2$/TaO_{\text{x}} memristor integration." Preprint (2026). [Experimental demonstration of ADC-bypass analog computation; physical $\Gamma_{st}$ pathway]
...
2. Methods/H: Added scale-normalization with Human HCP anchor ($k_{\text{max}}$/$k_{\text{null}}$ / 6.667); fixes 14/16 systems that had H > 1.0 in V23 (violating Axiom 1 boundedness).

3. Methods/M: Added $Q_{\text{rand}}$ = 0.02 correction and floor $\epsilon$ = 0.01 to prevent Sc = 0 for near-random networks (E. coli, MLP).
...
3. Theory/$\Gamma_{st}$: Added geometric interpretation of NMI(Ms,MT) as neural manifold alignment [55]; Theorem 1 cross-validation with $\gamma$*_{geo}=0.5 from independent coding-theoretic framework
...
6. Discussion: New section "Extension to higher-order networks" with S$c_{\text{HO}}$ formula and Betti number extension (deferred to companion paper)
...
2. $\eta_{\text{I}}$ values corrected: human $\eta_{\text{I}}$ 0.18$\rightarrow$3.67; GPT-4 $\eta_{\text{I}}$ 4 \\times 10$^{-}^7\rightarrow$8.8 \\times 10$^{-6}$ (per formula $\eta_{\text{I}}$=CST/$P_{\text{norm}}$; five-order-of-magnitude gap confirmed at 4.2 \\times 10$^5$)
