...
The critical coefficient $\alpha$ = ln($M_{\text{eff}}$) encodes node-level state diversity. The biological basis for $M_{\text{eff}}$ scaling with neural complexity has been illuminated by the evolutionary trajectory of synaptic architecture: from graded-potential proto-synapses in the last common ancestor of bilaterians (~600 Mya, $M_{\text{eff}}$ $\approx$ 13) through spiking multi-synaptic connections in insects (~500 Mya, $M_{\text{eff}}$ $\approx$ 32) to the multi-synaptic firing (MSF) neurons of mammalian cortex [56], which simultaneously encode spatial intensity via firing rate and temporal dynamics via precise spike timing, yielding $M_{\text{eff}}$ $\approx$ 32–64 (geometric mean $\approx$ 50). This evolutionary progression of $M_{\text{eff}}$—and correspondingly $\alpha$—is not a phenomenological fit but a direct consequence of the synaptic complexity accumulation over 600 million years of neural evolution [57]. $\alpha$ = ln($M_{\text{eff}}$) and is determined entirely by the physical signal transduction mechanism of the node, not by network topology or training. This creates a natural parameter family across biological and artificial systems. For binary digital logic, $M_{\text{eff}}$ = 2, giving $\alpha$_digital = ln(2) $\approx$ 0.69. For graded-potential neurons (non-spiking systems such as C. elegans and cnidarians), $M_{\text{eff}}$ $\approx$ 10–20 inferred from the ~40 mV dynamic range and ~3 mV voltage resolution of graded synapses [Liu et al., PNAS 2009; Lockery, Curr. Biol. 2009], giving $\alpha$_graded $\approx$ ln(13) $\approx$ 2.56. For spiking neurons with rate and temporal coding, Strong et al. [Science 1998] measured 3–6 bits per spike in cortical neurons ($M_{\text{eff}}$ = 2³–2⁶ $\approx$ 8–64, geometric mean $\approx$ 32), giving $\alpha$_spiking $\approx$ ln(32) $\approx$ 3.47. For human cortex with STDP and multi-frequency oscillations, conservative estimates (Rieke et al., Spikes, 1996) give $M_{\text{eff}}$ $\approx$ 50 and $\alpha$_cortical $\approx$ ln(50) $\approx$ 3.91. The six-fold gap between $\alpha$_digital and $\alpha$_cortical enters the exponent, creating a structural ceiling that parameter scaling cannot bridge.
...
Intel Loihi-2 ($CST = 0.7816$, Level I) is separately classified as Neuromorphic Hardware (NMH, $\alpha$ = ln(32) = 3.47), because its CMOS-implemented leaky integrate-and-fire neurons encode information through spike-timing dynamics rather than binary state transitions. The effective state multiplicity $M_{\text{eff}}$ $\approx$ 32 arises from the thermal-noise-limited membrane potential resolution ($\sigma$_V $\approx$ 0.6 mV against a ~20 mV dynamic range, yielding SNR $\approx$ 32 $\approx$ 2^5; see Methods), placing Loihi-2 at the low end of the biologically measured 3–6 bits/spike range [Strong et al., Science 1998]. This confirms the CST prediction that breaking the binary-digital $\alpha$-lock---not CMOS technology per se---is the first-generation hardware transition required to cross Level I.
