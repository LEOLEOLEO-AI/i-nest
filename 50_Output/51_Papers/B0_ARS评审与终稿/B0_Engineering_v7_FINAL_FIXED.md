...
<p align="center"><b>Fig. 1.</b> Graphical Abstract / 鍥惧舰鎽樿: Four-domain evidence synthesis and SDI three-stage diagram</p>
...
**CMOS technology scaling.** Horowitz''s canonical energy table [1] provides the foundational measurement. At 45 nm: a 32-bit integer addition costs 0.1 pJ; a 32-bit FP multiplication costs 3.7 pJ; an on-chip SRAM read (8 kB) costs 10 pJ; and an off-chip DRAM read costs 1,300—?,600 pJ. The ratio of DRAM access to FP multiply ranges from ~350× to ~700×. At more advanced nodes, Stillmaker and Baas (2017) updated these figures for 7 nm [3]: a 32-bit FP multiply drops to approximately 0.4 pJ, while an HBM2 access remains at approximately 300–80 pJ per bit — a ratio of ~750× to ~1,200×. The gap widens, not narrows, with process advancement.
...
|----------|------------------------|---------------------------|-------------------|
| General-purpose | SPEC CPU 2017 | 0.60—?.87 | Memory hierarchy (L1→扗RAM) |
| Intelligent computing (train) | GPT-3/LLaMA training | 0.30—?.50 | AllReduce collective |
| Intelligent computing (infer) | LLM serving (tensor parallel) | 0.25—?.40 | AllReduce + KV cache I/O |
| HPC | HPCG / MiniFE | 0.35—?.97 | MPI halo exchange + irregular access |
