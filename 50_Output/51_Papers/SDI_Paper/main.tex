...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{figures/Figure4_kappa_score_summary.png}
\caption{Experiment 5 (v14): SOC criticality validation summary. (A) Score distribution across 18 simulations (6 network$\times$rule combinations $\times$ 3 seeds): mean score 7.3/9, best 8/9. (B) Branching ratio $\kappa\in[0.92, 0.99]$ across all configurations---near-critical, consistent with in vivo recordings \citep{priesemann2014}. (C) PSD slopes $\approx -1.3$, confirming 1/f dynamics. (D) Functional decoding accuracy $>0.45$ across all configurations.}
\label{fig:exp5_summary}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/Figure6_CST_roadmap.png}
\caption{CST-predicted intelligence emergence roadmap for SDI-based neuromorphic systems. The four engineering generations (Gen1--Gen4) correspond to progressive unlocking of the Triple Lock: Gen1 breaks the $\alpha$-lock (memristive devices, $\alpha$: 0.69$\to$3.91); Gen2 raises $\Gamma_{st}$ via 3D integration; Gen3 coordinates spatiotemporal dynamics via SDI; Gen4 adds photonic interconnect for wafer-scale phase synchrony. Colored horizontal bands indicate the six natural-constant intelligence thresholds ($1/\sqrt{2}$, 1, $\varphi$, $e$, $\pi$, $\delta$).}
\label{fig:cst_roadmap}
\end{figure}
...
\paragraph{SDI as Physical Predictive Coding}
A deeper structural correspondence exists between SDI rules and predictive coding \citep{rao1999}---the leading biologically plausible alternative to backpropagation. Predictive coding posits that learning proceeds through \emph{local prediction errors} rather than globally coordinated error backpropagation: each neuron compares its actual activity against predictions from higher layers, and synapses update via local Hebbian plasticity $\Delta W \propto \varepsilon \times x_{\text{pre}}$. This maps precisely onto the SDI framework:

\begin{itemize}
  \item \textbf{E-L bonds} (STDP-fixed strong synapses) function as the \emph{representation pathway}---the top-down prediction channel that encodes stable world models.
  \item \textbf{E-S bonds} (plastic weak synapses) function as the \emph{error pathway}---the bottom-up channel that propagates prediction residuals.
  \item \textbf{Rule 1 (STDP)} implements local Hebbian plasticity, updating synaptic weights proportional to the coincidence of pre- and post-synaptic activity---mathematically equivalent to the predictive coding weight update rule.
  \item \textbf{Rule 3 (Homeostatic Scaling)} minimizes the energy $E = \tfrac{1}{2}\sum_l \|\varepsilon_l\|^2$ by adjusting weights to drive activation rates toward target levels, directly instantiating the energy minimization objective of predictive coding.
\end{itemize}

Crucially, this correspondence implies that the spatiotemporal coupling term $\Gamma_{st}$ in the CST theorem is \emph{physically maintained} by the SDI dynamics without requiring global computation: local STDP and homeostatic scaling continuously align structural connectivity patterns ($M_s$) with functional activation patterns ($M_T$), realizing $\Gamma_{st}$ through distributed synaptic plasticity. This provides the mechanistic foundation for why Gen1 memristive devices implementing physical STDP can achieve dynamic $\Gamma_{st}$---the core thermodynamic requirement for crossing the L1 intelligence threshold---without explicit coordination.
