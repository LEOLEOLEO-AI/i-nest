...
KEYWORDS = [
    # Scale Up/Out 基础 (8)
    'hierarchical network', 'distributed topology', 'scale-free network', 'small-world network',
    'data locality', 'load balancing', 'communication pattern', 'collective communication',
    
    # 复杂网络理论 (12)
    'complex network', 'community detection', 'modularity', 'network topology',
    'topology optimization', 'robustness', 'resilience', 'topology-aware', 'network-aware',
    'small-world', 'scale-free', 'criticality',
    
    # 训练推理融合 (6)
    'training-inference fusion', 'online learning', 'continual learning', 'dynamic neural network',
    'quantization', 'low-precision',
    
    # 能效优化 (5)
    'energy efficiency', 'power consumption', 'compute density', 'memory bandwidth', 'io bottleneck',
    
    # 现代网络架构 (8)
    'data center network', 'fattree', 'rdma', 'software-defined network', 'sdn',
    'network slicing', 'high-speed interconnect', 'deterministic network',
    
    # 神经形态与材料 (12)
    'neuromorphic', 'spiking neural', 'memristor', 'memristive', 'ferroelectric',
    '2d materials', 'fractional dynamics', 'nonlinear dynamics', 'avalanche',
    'connectome', 'elegans', 'c. elegans',
    
    # 系统灵活性 (8)
    'self-organization', 'emergent behavior', 'task parallelism', 'dynamic reconfiguration',
    'workload migration', 'decentralized control', 'collective intelligence', 'stigmergy',
    
    # 理论基础 (5)
    'distributed optimization', 'consensus algorithm', 'information theory', 'control theory',
    'communication complexity',
    
    # 原有保留核心 (6)
    'self-organized criticality', 'hemibrain', 'neural avalanche',
    'free energy principle', 'hebbian', 'stdp',
]
    ...
    for feed_url, feed_name in RSS_FEEDS:
        ...
        for paper in matched:
            if paper['id'] not in seen_ids:
                ...
                new_count += 1
        print(f'  [{feed_name}] {len(papers)} 篇 → {len(matched)} 篇命中 → {new_count} 篇新增')
