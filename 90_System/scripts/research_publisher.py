...
COMPILE_STATE = META / "knowledge_compile_state.json"
...
def compiled_knowledge_state():
    state = load_json(COMPILE_STATE, {"items": []})
    ...
    return {
        ...
        "items": [{
            ...
            "action": (item.get("candidate_actions") or ["人工确认具体行动"])[0],
            "file": item.get("file", ""),
        } for item in items[:8]],
        "review_note": f"60_MOC/12_Knowledge_Review_{TODAY}.md",
    }
...
def publish():
    ...
    payload = {
        ...
        "knowledge_review": compiled_knowledge_state(),
        "sources": {
    ...
    legacy = {
        ...
        "snapshot": {
            "total_md": payload["state"]["total_notes"],
            "tcc": payload["state"]["tcc_notes"],
            "inest": payload["state"]["inest_notes"],
            "inbox": payload["state"]["pending_papers"],
            "processing": payload["state"]["processing"],
            "output": payload["state"]["outputs"],
            "git_uncommitted": state.get("git", {}).get("uncommitted", 0),
        },
        "knowledge_review": payload["knowledge_review"],
    }
    DATA_FILE.write_text(
        ...
        "window.VAULT_DATA = " + json.dumps(legacy, ensure_ascii=False) + ";\n",
        encoding="utf-8",
    )
