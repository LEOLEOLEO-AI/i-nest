"""SDI v22 — 自适应θ节律 + FEP全局稳态 + 外部数据闭环驱动
==========================================================
...
OUT_DIR   = "D:/Obsidian/phase1_workspace/v22_results"
...
class SDI_v22:
    ...
    def step(self):
        ...
        # v22: 自适应θ + 键固化（每10步计算一次，避免每微步都做全图遍历）
        if self.t % 10 == 0:
    ...
    def run(self):
        print(f"\nv22仿真: {N_STEPS}步 | 自适应θ + FEP稳态 + 结构化数据驱动")
        print("-"*70)
    ...
    def plot_results(self, logs, sigma, alpha, C, L, el_ratio, F_final):
        ...
        fig.suptitle("SDI v22 — 自适应θ节律 + FEP全局稳态 + 外部数据闭环",
        ...
        plt.tight_layout()
        plt.savefig(f"{OUT_DIR}/sdi_v22_main.png", dpi=150, bbox_inches="tight")
        plt.close()
        print(f"  主图已保存 → {OUT_DIR}/sdi_v22_main.png")
        ...
        print(f"  结果已保存 → {OUT_DIR}/v22_results.json")

# ============ 运行 ============
if __name__ == "__main__":
    print("="*60)
    print("SDI v22 — 自适应θ + FEP稳态 + 外部数据闭环")
    print("="*60)
    sim = SDI_v22()
