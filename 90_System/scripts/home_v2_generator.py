...
def generate():
    ...
    lines += [f"| 知识库 Markdown | {vault.get('total_md', count('')):,} | {link('60_MOC/00_Processing_Pending_Review', '健康与待复核')} |", f"| TCC 知识 | {vault.get('tcc_30', count('30_TCC')):,} | {link('30_TCC/TCC_Master_Index', 'TCC 总索引')} |", f"| iNEST 知识 | {vault.get('inest_40', count('40_iNEST')):,} | {link('40_iNEST/iNEST_Master_Index', 'iNEST 总索引')} |", f"| Processing | {vault.get('processing_20', count('20_Processing')):,} | {link('20_Processing', '处理中')} |", f"| 论文/专利/代码/指南 | {vault.get('output_50', count('50_Output')):,} | {link('50_Output', '成果区')} |", "", "## 研发主线", "", "| 方向 | 理论攻关 | 技术研究 | 工程落地 | 成果出口 |", "|---|---|---|---|---|", f"| TCC | {link('30_TCC/31_Theory', '拓扑中心计算与 R/T/C 基线')} | {link('30_TCC/32_Technologynology', 'SDI / Chiplet / OneFabric')} | {link('30_TCC/33_Engineering', 'RTL / CST / 原型')} | {link('50_Output/51_Papers/00_Paper_Versions_Index', '论文版本总表')} |", f"| iNEST | {link('40_iNEST/41_Theory', '复杂网络与涌现智能')} | {link('40_iNEST/42_Technologynology', 'SNN / 器件 / 学习规则')} | {link('40_iNEST/43_Engineering', '仿真与工程实现')} | {link('50_Output/51_Papers/00_Paper_Versions_Index', '论文版本总表')} |", "", "## 外部知识编译待确认", "", "> 这些条目是外部资料的处理结果，不是团队原创成果；确认前不会进入正式任务计划或成果统计。", ""]
    if review_items:
        lines += ["| 条目 | 方向 | 证据 | 下一步 |", "|---|---|---|---|"]
