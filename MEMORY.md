...
### 工程目录
- **硬件工程**：`/home/work/.openclaw/workspace/iNEST_HW_Engineering/`
- **仿真研究**：`/home/work/.openclaw/workspace/iNEST_Sim_Research/`

### SDI四规则 v6（工程基准版，已锁定）
| 规则 | v6名称 | 核心改进 | 关键参数 |
|------|--------|---------|---------|
| Rule1 | SA-STDP | 惊讶度调制学习速率 | α=0.4, β=0.3, τ=20ms |
| Rule2 | PE-Syn | 预测误差引导+度标度律 | k∝N^0.14, EL_target=0.29 |
| Rule3 | BF-Homeo | BCM+FEP双层稳态 | θ_BCM=[7,8], EL∈[15%,35%] |
| Rule4 | EC-Pruning | EL区间约束竞争性修剪 | P_PRUNE自适应, Q>0.6 |

### 已启动的IP RTL代码
- `IP1 SA-STDP`：`01_IP_Design/rule1_stdp/stdp_core.v`（256突触/core）
- `IP3 BF-Homeo`：`01_IP_Design/rule3_bcm/bcm_homeo_core.v`（256节点）

### v22-v31 已证明/未证明总结
**已证明**：
- σ可从任意初始拓扑涌现小世界（v27/v28，5/5 PASS）
- BCM精确控制EL=29%（生物参考）
- 趋光趋化行为涌现（v29，approach_ratio=0.97）
- 跨脑区功能耦合（v30，7脑区，MB↔SEZ=0.93）
- 多智能体协作（v31）

**未证明（下一批）**：
- σ与CST六阈值的严格因果关系 → v32
- σ各区间的功能能力单调对应 → v32-v34
- 硅基约束（8bit精度/μs延迟）下四规则有效性 → v35
- 卫星编队σ分布 → v36

### MVP产品路线
- MVP1：单芯片262K突触（2027-Q2流片）
- MVP2：多芯片1M突触互连板（2027-Q4）
- MVP3：星载智能模块（2028-Q4）

## 红线原则：禁止凑数（2026-06-08，刘教授明确）

**任何中间结果、中间数据，必须来自实际程序运行的真实输出。**

- 禁止用"合理估算"、"预期范围"、"定性预测"填充定量数值
- 禁止用旧定义下的测量值代入新定义的公式
- 禁止在参数待测时给出"验证通过"的结论
- 未实测的量必须明确标注"待测"，不得给出具体数字
