# 全库健康自检

> 生成: 2026-08-27  ·  链接解析贴近 Obsidian 行为

- 笔记总数(可链接目标): **10828**
- 缺 frontmatter 笔记: **936**
- 真正断链(目标不存在): **3956**

## 断链样本(按被引用次数排序, 优先补全)

- (×283) `[[temporalComplexity]]`
- (×202) `[[soc]]`
- (×192) `[[Neuromorphic_computing]]`
- (×160) `[[criticality]]`
- (×94) `[[inest]]`
- (×58) `[[Spiking_neural_network]]`
- (×56) `[[tcc]]`
- (×42) `[[connectome]]`
- (×32) `[[Spike_timing_dependent_plasticity]]`
- (×32) `[[heterogeneous_integration]]`
- (×29) `[[GetNote_20260606_100554_kb_project-guide_getnote_1902402582405550008_晶上自演化复杂网络的基础理论_关键技术与应用验证_项目群_]]`
- (×18) `[[GetNote_20260606_100554_kb_project-guide_getnote_1907691712224232464_海河实验室晶上先导项目检查]]`
- (×18) `[[tools]]`
- (×17) `[[GetNote_20260606_100554_kb_paper-ideas_getnote_1906500077841850152_从冯诺依曼到网络中心_计算范式迁移的第一性原理综述]]`
- (×16) `[[Universal Scaling Laws for Intelligence Emergence from Device-Level Nonlinearities  A Renormalization Group Approach]]`
- (×13) `[[2026-06-28_Getnote_2026-06-28_拓扑中心计算范式架构研究与液态拓扑生成式计算原型验证项目指南 8.0]]`
- (×13) `[[GetNote_20260606_100554_kb_DARPA-research_getnote_1896185525975594536_DARPA传感器_电子战与通信领域深度研究报告_下册__能力与技术预测_2025-2035_]]`
- (×13) `[[GetNote_20260606_100554_getnote_1911898566117388992_NEST 智能涌现研究进展与工程落地全景]]`
- (×13) `[[GetNote_20260606_100554_kb_iNEST_getnote_1899812166295444000_清华团队类脑计算突破]]`
- (×13) `[[Soc]]`
- (×12) `[[The_Rise_of_Neuromorphic_Computing_How_Brain-Inspired_AI_is_Shaping_the_Future_in_2025]]`
- (×11) `[[GetNote_20260606_100554_kb_iNEST_getnote_1887266194235479456_晶上大脑基础研究课题]]`
- (×11) `[[INEST]]`
- (×10) `[[GetNote_20260606_100554_kb_iNEST_getnote_1887095535857468536_ResNEST论文修订]]`
- (×10) `[[82岁江泽民在2008年发表论文指出：发展智能化，机器学习将有所作为…… (1)]]`

- 孤儿笔记(无入链): **2444**