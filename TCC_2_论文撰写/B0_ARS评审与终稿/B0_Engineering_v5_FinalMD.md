...
# 《软件定义互连的范围综述：概念、证据与面向下一代智能计算的液态统一架构》

Qinrang Liu (刘勤让)?′*, et al.
? TCC iNEST Research Group
* Corresponding author. E-mail: qinrangliu@gmail.com

**Version:** v5 (Final) | **Article type:** Review | **Target:** *Engineering* — Special Issue on Sustainable Intelligent Computing | **Word budget:** ≤10,000
...
6. The scoping review methodology synthesizes evidence across four computing scenarios, establishing that data-movement optimization, rather than operator acceleration, is the critical path to sustainable intelligent computing.
...
本文提出一个基于第一性原理的核心命题：任何计算过程均可通过弱耦合分解为两类原语——算子与数据移动——且数据移动构成了根本性瓶颈。通过对通用计算、智能计算、高性能计算和信号处理四种典型场景的系统分析，本文确立了六项发现。第一，硬件原子算子在所有场景中收敛于不超过十种原语的有限集合；所有高阶数学均可通过Weierstrass逼近和CORDIC方法约简，使得算子不再值得进一步优化。第二，数据移动模式可以通过十一种跨越性元原语及其成本模型进行形式化，从而实现编译时优化。第三，软件定义互连提供了一种将路由从设计时固定提升为运行时可编程的机制，且其效益阈值条件可被形式化刻画。第四，“液态统一架构”整合了标准化算子、数据移动元原语和SDI互连结构，实现了从以节点为中心到以网络为中心的计算范式迁移，并将六种现有的非冯·诺依曼路径统一于单一框架之内。

本文最后将这一范式迁移置于可持续智能计算的背景下讨论，并提出五项可经验检验的研究议程。前进的道路不在于更快的处理器，而在于更智能的互连结构——直接攻克那90%，而非在10%上迭代优化。

**关键词：** 软件定义互连；数据移动；液态统一架构；非冯·诺依曼计算；元原语；范围综述

**Keywords:** Software-defined interconnect; data movement; liquid unified architecture; non-von-Neumann computing; meta-primitives; scoping review
...
<p align="center"><b>Fig. 2.</b> Data movement energy fractions (ρ) across four computing scenarios. Stacked bars show data movement vs. computation energy. The dashed line at ρ = 0.5 demarcates the crossover point beyond which data movement dominates total energy. / 四种计算场景下的数据移动能耗占比可视化</p>
...
<p align="center"><b>Fig. 3.</b> Liquid unified architecture — five-layer diagram: Compute Tile Array, SDI Fabric, Meta-Primitive Scheduler, Memory Hierarchy, and Host Interface. / 液态统一架构五层示意图</p>
...
This review has established six findings:
