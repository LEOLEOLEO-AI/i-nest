#!/usr/bin/env python3
"""
SDI 实验五 v10：双重G校准 + 加强适应期（三阶段协议最终版）
...
# ★ v10增强泊松输入
LAMBDA_STIM  = 0.40   # 适应期激活率（v9的2.7倍）
...
def plot_results(all_results):
    ...
    fig.suptitle(
        'SDI Exp5 v10: Dual-Calibration (kappa + size) + Enhanced Adaptation\n'
    ...
    plt.savefig(f'{OUT}/exp5_v10_avalanche_results.png', dpi=150, bbox_inches='tight')
    plt.close()
    print(f"Figure: {OUT}/exp5_v10_avalanche_results.png")
...
def main():
    print("="*70)
    print("SDI Experiment 5 v10 — Dual-Calibration + Enhanced Adaptation")
    print(f"  Phase1 Adapt  : {N_ADAPT}步 | λ_stim={LAMBDA_STIM} T_on={T_STIM_ON} + STDP(strong)")
    ...
    out_path = f'{OUT}/exp5_v10_avalanche_results.json'
