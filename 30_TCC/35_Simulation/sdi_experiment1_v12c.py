...
I_NOISE   = 0.0    # mV/step，不用背景噪声（刺激驱动型）
A_LTC     = 30.0   # mV，LTC驱动幅度（膜电位上限偏移）
T_BIN     = 4.0    # ms，雪崩检测时间窗口
T_STIM    = 50.0   # ms，刺激间隔（让上次雪崩完全熄灭）
# 突触时间常数（与化合键类型耦合）
...
class SDI_LTC:
    def __init__(self, name, sp, seed):
        ...
        wl=np.random.uniform(.08,.28,len(sl3))  # 亚临界初始权重
        ...
        self.sens=np.arange(ns)   # 感觉神经元索引
        self.pats=[]              # 保留兼容
        self.cp=0; self.pc=0
    ...
    def _stim_seeds(self):
        """单点刺激（模拟体外皮层片层电极）：
        每 T_STIM ms 随机选1个感觉神经元刺激，产生单次雪崩
        等待50ms让上次雪崩完全熄灭再下次刺激"""
        stim_interval = int(T_STIM / DT)
        if self.t % stim_interval != 0:
            return []
        # 随机选1个感觉神经元（轮换模式）
        if self.pc >= len(self.sens):
            self.pc = 0
        seed = int(self.sens[self.pc])
        self.pc += 1
        return [seed]  # 单点刺激
    ...
    def ltc_step(self, t_ms):
        ...
        self.V[seeds] = V_TH + 2.0  # 直接设到阈值以上（刺激驱动）
        # 无自发背景激活（纯刺激驱动型）
...
def run_species(name, sp):
    ...
    for m in ['sigma','C','L','alpha','el']:
        ...
        print(f'  {"✅" if final[f"pass_{m}"] else "❌"} '
              f'{m:6s}: {mu:.3f}±{sd:.3f}  (target {tgt_str})') if mu is not None else print(f'  ❌ {m:6s}: N/A  (target {tgt_str})')
...
def main():
    ...
    def fix(o):
        ...
        return o
    with open(f'{OUT}/exp1_v12c_results.json','w') as f:
    ...
    plt.tight_layout()
    plt.savefig(f'{OUT}/exp1_v12c_convergence.png',dpi=130,bbox_inches='tight')
    plt.close()
    print(f'Results → exp1_v12c_results.json')
    print(f'Plot → exp1_v12c_convergence.png')
    print('DONE')
