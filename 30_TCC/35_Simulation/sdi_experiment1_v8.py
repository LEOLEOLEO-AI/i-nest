...
SPECIES = {
    ...
    'Rat_Cortex':     {'N':73,  'k':14, 'k_init':12, 'p_init':0.08, 'sf':0.15, 'init':'ws',
        ...
        'tgt':{'sigma':(1.5,None),'C':(.25,None),'L':(1.5,3.),'alpha':(1.5,2.5),'el':(.15,.28)}},
    'Mouse_Cortex':   {'N':112, 'k':14, 'k_init':12, 'p_init':0.10, 'sf':0.15, 'init':'ws',
        'bio':{'sigma':3.2,'C':.45,'L':1.8,'alpha':2.1,'el':.20},
        'tgt':{'sigma':(2.0,None),'C':(.30,None),'L':(1.5,3.),'alpha':(1.5,2.5),'el':(.15,.28)}},
...
def main():
    ...
    def fix(o):
        ...
        return o
    with open(f'{OUT}/exp1_v8_results.json','w') as f:
    ...
    plt.tight_layout()
    plt.savefig(f'{OUT}/exp1_v8_convergence.png',dpi=130,bbox_inches='tight')
    plt.close()
