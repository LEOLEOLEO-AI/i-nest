...
POISSON_RATE  = 1.5    # Hz，泊松背景噪声激活率（每感觉神经元）
...
class LIF_SDI_v15:
    ...
    def triadic_closure(self, t_now):
        ...
        # 边数上限：防止过度稠密导致L偏低（L目标≥2.0）
        cm_all = (self.bt==0)|(self.bt==1)
        max_edges = self.N * self.sp['k_avg'] * 3  # 最大边数=初始的3倍
        if cm_all.sum() >= max_edges:
            return  # 已达上限，跳过三元闭合
    ...
    def run_once(self):
        ...
        n_spikes=0; stim_count=0; last_poisson_t=0.

        # 预生成泊松激活事件（高效：O(n_events) 而非 O(T/dt * N_sens)）
        for si in self.sens:
            t_p = 0.
            while t_p < T_SIM:
                t_p += np.random.exponential(1000. / POISSON_RATE)
                if t_p < T_SIM:
                    heapq.heappush(heap, (t_p, int(si) + N * 1000))  # 用偏移区分泊松事件
        # 注：泊松事件 node_id >= N*1000，在 fire() 里识别并处理
        ...
        while t_now<T_SIM:
            ...
            # 周期结构刺激（低频，辅助泊松噪声维持感觉模式）（低频，维持感觉模式）
            ...
            while heap and heap[0][0]<=t_now+1e-6:
                ...
                # 识别泊松事件（node_id >= N*1000）
                if i >= N * 1000:
                    real_i = i - N * 1000
                    if t_fire >= self.ref_end[real_i]:
                        self.V[real_i] = V_TH + 1.5
                        self.V_t[real_i] = t_fire
                        ts = self._next_spike(real_i, t_fire)
                        if ts < T_SIM and np.isfinite(ts):
                            heapq.heappush(heap, (ts, real_i))
                    continue
...
def main():
    ...
    def fix(o):
        ...
        return o
    with open(f'{OUT}/exp1_v17b_results.json','w') as f:
    ...
    plt.suptitle('SDI Exp1 v17 — 事件驱动LIF + BA初始图 + 三元闭合 + 30s仿真\n'
                 '随机初始 → SDI规则演化 → 生物复杂网络特性',
                 fontsize=11,fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'{OUT}/exp1_v17b_convergence.png',dpi=130,bbox_inches='tight')
    plt.close()
    print('Results → exp1_v17b_results.json')
    print('Plot → exp1_v17b_convergence.png')
    print('DONE')
