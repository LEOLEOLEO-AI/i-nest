...
SPECIES = {
    ...
    'Rat_Cortex':     {'N':73,  'k':14, 'k_init':12, 'p_init':0.15, 'sf':0.15,
        'bio':{'sigma':3.,'C':.42,'L':1.9,'alpha':2.,'el':.18},
        'tgt':{'sigma':(2.5,None),'C':(.30,None),'L':(1.5,3.),'alpha':(1.5,2.5),'el':(.15,.28)}},
    'Mouse_Cortex':   {'N':112, 'k':14, 'k_init':12, 'p_init':0.18, 'sf':0.15,
...
def main():
    ...
    def fix(o):
        ...
        return o
    with open(f'{OUT}/exp1_v6_results.json','w') as f:
    ...
    plt.tight_layout()
    plt.savefig(f'{OUT}/exp1_v6_convergence.png',dpi=130,bbox_inches='tight')
    plt.close()
