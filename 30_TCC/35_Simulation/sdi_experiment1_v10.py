...
class SDI_Net:
    ...
    def run_once(self):
        ...
        for step in range(N_STEPS):
            ...
            # 目标：让网络分支比 κ → KAPPA_TARGET（SOC临界点）
            # κ = 平均雪崩大小增长率
            # 实现：用最近100步雪崩大小中位数估算κ，
            #       κ<目标→上调兴奋突触权重，κ>目标→下调
            # ======================================================
            if step % SCALING_INT == 0 and step > 0 and len(avs) >= 20:
                recent = avs[-50:]
                kappa_est = float(np.median(recent)) / max(1, float(np.median(avs[-100:-50])) if len(avs)>=100 else 1.0)
                kappa_hist.append(kappa_est)
                # 兴奋性短时程键（E-S，bt==0）做缩放
                exc_s = self.bt == 0
                if exc_s.sum() > 0:
                    if kappa_est < KAPPA_TARGET:
                        # 亚临界：上调权重，让雪崩更大
                        scale = 1.0 + SCALING_RATE * (KAPPA_TARGET - kappa_est) / KAPPA_TARGET
                        self.w[exc_s] = np.clip(self.w[exc_s] * scale, 0.01, 1.0)
                    elif kappa_est > KAPPA_TARGET * 1.1:
                        # 超临界：下调权重，防止爆发
                        scale = 1.0 - SCALING_RATE * (kappa_est - KAPPA_TARGET) / KAPPA_TARGET
                        self.w[exc_s] = np.clip(self.w[exc_s] * scale, 0.01, 1.0)
                    self._rb()
        cm=(self.bt==0)|(self.bt==1)
...
def main():
    ...
    def fix(o):
        ...
        return o
    with open(f'{OUT}/exp1_v10_results.json','w') as f:
        json.dump(fix(all_r), f, indent=2)
    print(f'\nResults → exp1_v10_results.json')
    ...
    plt.suptitle('SDI Experiment 1 v10 — 7-Species Cross-Kingdom Universality\n'
                 f'(5 random seeds each | neuron-level + mesoscale★ | Hill MLE α)',
                 fontsize=11, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'{OUT}/exp1_v10_convergence.png', dpi=130, bbox_inches='tight')
    plt.close()
    print('Plot → exp1_v10_convergence.png')
    print('DONE')
