"""
SDI 实验十一 — 异质LIF v2：修复过饱和 + τ-结构对齐
==========================================================
修复实验十的两个问题：
  1. λΦ=1.0过饱和 → top-K稀疏激活 + 抑制-激发平衡
  2. Γst下降 → 社区内τ同构、社区间τ异构

文献依据（新增）：
  Vreeswijk & Sompolinsky 1996 Science 274:1724
    平衡态网络：E/I平衡 → 稀疏激发，λ_eff→0.95而非1.0
  Murray et al. 2014 Nat.Neurosci. 17:1661
    同一脑区内INT相关性ρ=0.6-0.8 → 社区内τ相似
  Perez-Nieves et al. 2021 Nat.Commun.
    异质τ对数正态分布，跨区域τ差异显著
"""
...
BASE    = '/vault/sdi_sim'
OUT     = os.path.join(BASE, 'exp11_hetLIF_v2_results.json')
...
# ── 异质LIF v2参数（修复）────────────────────────────────
TAU_MU    = np.log(20)  # 几何均值≈20步
TAU_SIGMA = 1.0         # 跨3个数量级（Murray 2014）
TAU_MIN   = 5.0
TAU_MAX   = 200.0

# 修复1：稀疏激活（Vreeswijk 1996）
SPARSE_K   = 0.12   # 每步保留top-12%激活节点（生物皮层稀疏编码）

# 修复2：社区内τ同构系数（Murray 2014）
# 社区内节点τ从同一窄分布采样，跨社区τ差异显著
INTRA_COMM_SIGMA = 0.3   # 社区内τ的对数标准差（窄分布）
INTER_COMM_SPREAD = 1.5  # 社区间τ中心值的扩展因子
...
def get_initial_communities(W):
    ...
    try:
        ...
        N=W.shape[0]
        return np.arange(N)%5, 5

def make_tau_structured(N, comm_labels, n_comms, rng):
    """
    社区感知的τ分配（修复2）
    Murray 2014：同一脑区INT相关性ρ=0.6-0.8 → 社区内τ窄分布
    不同脑区（社区间）τ中心值差异显著
    """
    # 每个社区分配一个不同的τ中心（对数尺度均匀分布）
    comm_centers = np.linspace(np.log(TAU_MIN*1.5),
                               np.log(TAU_MAX*0.7),
                               max(n_comms, 1))
    ...
    for i in range(N):
        ...
        # 社区内窄分布（Murray: ρ=0.6-0.8 → σ_intra=0.3）
        log_tau_i = rng.normal(comm_centers[ci], INTRA_COMM_SIGMA)
        tau[i] = float(np.clip(np.exp(log_tau_i), TAU_MIN, TAU_MAX))
...
# ── 修复后的异质LIF激活 ────────────────────────────────────
def activate_hetLIF_sparse(W, h, tau, rng, n_seeds=4):
    """
    异质LIF + 稀疏激活（修复过饱和）
    
    修复1 - Vreeswijk 1996：
      驱动→LIF更新→top-K稀疏截断
      保持λ≈0.95（临界态），不允许λ=1.0（过饱和）
    """
    N = W.shape[0]
    leak = (1.0 - 1.0/tau).astype(np.float32)

    # 稀疏外部驱动（感觉输入）
    n_in = max(n_seeds, int(N*0.05))
    drive = np.zeros(N, dtype=np.float32)
    drive[rng.choice(N, n_in, replace=False)] = rng.uniform(0.3, 0.7, n_in)

    # LIF更新
    h_new = leak * h + np.tanh(W @ h + drive)

    # top-K稀疏截断（Vreeswijk 1996：E/I平衡→稀疏激发）
    K = max(3, int(N * SPARSE_K))
    if h_new.max() > 0:
        thresh = np.partition(h_new, -K)[-K] if K < N else 0
        h_new = np.where(h_new >= thresh, h_new, 0.0)
...
def run(name, cfg, seed):
    ...
    # 修复2：社区感知τ分配
    comm_labels, n_comms = get_initial_communities(W)
    tau = make_tau_structured(N, comm_labels, n_comms, rng)
    Theta = compute_Theta(tau)
    print(f"  τ: min={tau.min():.1f} max={tau.max():.1f} mean={tau.mean():.1f} "
          f"n_comms={n_comms} Θ={Theta:.3f}")
    ...
    for step in range(N_STEPS):
        h   = activate_hetLIF_sparse(W, h, tau, rng)
        act = h.copy()
...
if __name__=='__main__':
    ...
    print("="*65)
    print("SDI 实验十一 — 异质LIF v2：修复过饱和 + τ-结构对齐")
    print(f"  修复1: top-{SPARSE_K*100:.0f}%稀疏激活 (Vreeswijk 1996 Science)")
    print(f"  修复2: 社区内τ同构σ={INTRA_COMM_SIGMA}(Murray 2014 Nat.Neurosci.)")
    print("="*65)
    ...
    print(f"\n{'网络':<15}{'Θ':>6}{'Tc':>6}{'Γst':>7}{'CST':>8}{'IIL':>12}")
    print("  "+"-"*55)
    summary={}
    for net,rl in by.items():
        ...
        ilv=fins[0].get('IIL','?')
        print(f"  {net:<15}{th:>6.3f}{tc:>6.3f}{gst:>7.3f}{cst:>8.3f}{ilv:>12}")
    ...
    e10 = {'WS_300':0.415,'C_elegans':0.543,'Human_HCP':1.708}
    for net in summary:
        c11=summary[net]['CST']
        print(f"  {net}: 实验九={e9.get(net,0):.3f} → "
              f"实验十={e10.get(net,0):.3f} → "
              f"实验十一={c11:.3f}  [{summary[net]['IIL']}]")
