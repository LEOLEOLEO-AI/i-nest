...
class SDI_v5:
    def __init__(self, name, sp):
        ...
        init_type = sp.get('init', 'ws')
        if init_type == 'ba':
            # Barabási-Albert 无标度图：hub结构天然高σ
            # 用优先连接（preferential attachment）建图
            m = max(2, k//2)  # 每个新节点连接 m 个已有节点
            es = set()
            # 初始小团
            ...
            for new_node in range(min(m+1, N), N):
                # 优先连接概率 ∝ degree
                probs = deg_arr[:new_node].copy()
                probs = probs / probs.sum()
                targets = np.random.choice(new_node, size=min(m, new_node),
                                           replace=False, p=probs)
                for t in targets:
                    t = int(t)
                    es.add((new_node, t)); es.add((t, new_node))
...
def main():
    ...
    def fix(o):
        ...
        return o
    with open(f'{OUT}/exp1_final_results.json','w') as f:
    ...
    plt.suptitle('SDI Experiment 1 — Cross-Species Universality\nRandom Init → SDI Rules → Small-World Convergence\n(Neuron-level: C.elegans/Larval_Droso/Macaque 5/5 | Mesoscale: Rat/Mouse)',
                 fontsize=11,fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'{OUT}/exp1_final_convergence.png',dpi=130,bbox_inches='tight')
    plt.close()
