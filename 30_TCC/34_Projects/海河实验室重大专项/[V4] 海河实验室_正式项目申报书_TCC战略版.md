...
## 1.3 国家战略意义：换道超车与范式定义权

本项目作为海河实验室**软件定义晶上系统（SDSoW）**战略方向的先导与奠基性项目，承担三重国家战略使命：

1. **打破国际生态垄断**：跳出NVIDIA NVLink与InfiniBand定义的封闭互连框架，以**拓扑可重构指令集（TR-ISA）**建立中国主导的TCC-Link互连标准。在UCIe尚未覆盖的协议层/语义层实现"兼容+超越"——物理层充分兼容国际标准，协议层实现颠覆性创新。

2. **实现换道超车**：在Chiplet与晶圆级集成赛道上，将竞争维度从"拼算力面积"转向"拼网络架构复杂度"。传统路线是"更大芯片→更多算力"，TCC路线是"更聪明的网络→更高的有效算力"。**以拓扑的智慧替代面积的堆砌**，为我国下一代算力基础设施提供颠覆性底层架构方案。

3. **抢占范式定义权**：1945年冯·诺依曼用ENIAC验证"存储程序"概念，定义了此后70年的计算范式。本项目以同样逻辑——用FPGA验证TCC范式——争夺**第二代计算范式（拓扑中心计算）的定义权**。范式成立后再推进流片与晶圆级集成，分步降低战略风险。

**本项目不追求9颗芯粒的全定制流片。战略逻辑清晰：范式先行，工程跟进。**
...
**物理意义**：该定理证明，Route与Transform是分布式计算的两个正交基。任何一个"计算"都可以被分解为"数据如何在节点间流动（Route）"和"节点对数据做了什么（Transform）"。当硬件可以分别加速这两类基元操作时，任意分布式计算均可映射到同一套硬件上。
...
**工程推论**：当硬件拥有可重构的物理拓扑时，同一组物理连线可以在AI推理中承担AllReduce通信，在雷达信号处理中承担FFT蝶形变换——连线完全不变，仅需切换端点的操作模式。

## 2.4 元拓扑与SDI化合键——拓扑可重构的代数基础

Route-Transform同构定理证明了"通信≈计算"的数学事实，但尚未回答一个更根本的问题：**物理拓扑的重构本身是否有一个可计算的代数系统？**

答案是肯定的。我们建立了**元拓扑-SDI化合键代数架构**（Meta-Topology & SDI-Bond Algebra），作为TCC范式拓扑可重构性的数学基础：

**3类元拓扑（不可约化基底）**：

| 元拓扑 | 图论表示 | 物理语义 | 对应通信模式 |
|--------|---------|---------|------------|
| 有向边 | $P_2$ | 点对点单向传输 | MOVE |
| 星型图 | $K_{1,n}$ | 一对多扇出/扇入 | CAST（广播）/ FUSE（汇聚） |
| 环型图 | $C_n$ | 循环流水线 | 流水线AllReduce / 环形Scan |

**5类SDI化合键（拓扑合成操作）**：

| 化合键 | 图论操作 | 物理语义 |
|--------|---------|---------|
| 平行键 | 笛卡尔积 $G \square H$ | 正交维度的独立扩展（如2D Mesh = Ring □ Ring） |
| 交叉键 | Kronecker积 $G \otimes H$ | 分形自相似扩展（如Butterfly = $P_2 \otimes P_2 \otimes \cdots$） |
| 融合键 | 强乘积 $G \boxtimes H$ | 全连通合成（如Crossbar = Star ⊠ Star） |
| 叠加键 | 图并集 $G \cup H$ | 异构拓扑共存 |
| 替换键 | 图替换 $G[H_v]$ | 层级化模块嵌入 |

**核心定理**：基于3种元拓扑和5种SDI化合键，可以数学推演并生成所有5类通信原语（AllReduce/AlltoAll/ReduceScatter/AllGather/Broadcast）的最优物理拓扑。这意味着：**TCC的拓扑重构不是任意的，而是受一个闭合的代数系统严格约束的**——正如布尔代数约束了数字逻辑的合成方式，元拓扑-SDI化合键代数约束了物理网络的合成方式。

## 2.5 拓扑自由能原理——拓扑的自组织优化

我们将统计物理引入网络拓扑设计，提出**拓扑自由能原理**（Free Energy Principle for Topology）：

**核心思想**：系统运行时的最优拓扑构型，是使得"拓扑重构动能"与"通信延迟势能"所构成的**网络拉格朗日作用量**最小化的解：
...
通过求解**网络欧拉-拉格朗日方程**，液态硬件能够在不同计算负载下自发寻找最优互连结构，实现类似生物神经网络的自适应演化。该原理为SDI控制器的运行时拓扑调度算法提供了严格的数学基础。

## 2.6 Kronecker分形扩展——破解互连扩展墙

分布式计算面临的核心工程困境是**互连扩展墙**——随着节点数N增加，通信开销呈$O(N)$甚至$O(N^2)$增长，集群扩展效率持续下降。

基于SDI化合键中的**Kronecker积操作（交叉键）**，TCC范式提供了一种根本性的解决方案——**分形拓扑扩展**（Fractal Scaling）：

```
单个Die内部:   4个计算模块以Ring ($C_4$) 形式互连
     ↓ Kronecker积
单个Chiplet:   4个Die以Ring形式互连 → 形成 $C_4 \otimes C_4$ 分形结构
     ↓ Kronecker积
板卡级:        4个Chiplet以Ring形式互连 → 形成 $C_4 \otimes C_4 \otimes C_4$
```

**核心性质**：基于Kronecker积的拓扑扩展具有**自相似性**——每一级的互连规则完全相同，通信复杂度的增加呈理想的对数级（$O(\log N)$而非$O(N)$）。这从根本上破解了大规模AI训练的互连扩展墙，使TCC架构从芯片级到晶圆级再到机柜级实现无缝水平扩展。
...
TCC理论经历了从"5+4"（5通信+4算子，V1版）到**TCC-16**（通信4+计算4+流动3，v1.0最终版）的严格理论提炼。演化逻辑如下：

| 版本 | 体系 | 核心变化 |
|------|------|---------|
| V1 (5+4) | 通信原语5 + 计算原语4 = 9个 | ReduceScatter可被FUSE+SWAP组合表达，违反正交最小性 |
| **TCC-16 v1.0** | **通信4 + 计算4 + 流动3 = 11个** | 剔除ReduceScatter；新增流动原语（MOVE/LINK/TICK）；形成三类完备体系 |

TCC-16遵循三条设计原则：**代数完备性**（任意分布式计算可在有限步内由原语组合表达）、**正交最小性**（任何一个原语不能被其余原语在O(1)或O(log N)步内等价替代）、**硬件可映射性**（每个原语对应面积≤50K LUT等效门的独立RTL IP核）。

命名规则：统一前缀 `tcc.`，后接四字母可发音英语动词助记符（如FUSE读作/fjuːz/，SCAN读作/skæn/）。
...
**完备性验证**：TCC-16已通过7个典型场景（LLM推理、视频目标检测、雷达DBF、SpMV、FFT、MoE训练、图神经网络）的逐原语覆盖验证，覆盖率100%。关键发现：**MAPS和LINK是唯二在所有场景中都出现的原语**——"逐元素映射"和"拓扑控制"是TCC范式的两大绝对核心。
...
Phase 2 (2029-2031): ASIC产品化 —— 端侧双模芯片
  ├── 基于验证IP的28nm/12nm ASIC流片（扩展至TCC-16全集）
  ├── TCC-Edge-ASIC产品原型（RT-Infer-30边缘推理卡）
  ├── 孵化公司独立运营
  └── TRL-7（工程样机）
...
TCC-16原语库定义了三类原语。Phase 1的FPGA原型聚焦于验证原语的最小完备子集——恰好是"3个数据原语 + 1个控制原语"：
...
**定位**：AI推理骨干网络和DBF波束成形权重的共享矩阵乘加速器。
...
**定位**：执行所有逐元素操作——FFT旋转因子乘法、神经网络激活函数、归一化、阈值过滤。在TCC-16完备性验证中，MAPS是唯二在所有7个场景中都出现的原语之一。
...
**定位**：液态硬件的"神经中枢"，TCC-Link标准的硬件实现载体。LINK是TCC-16流动原语的核心——管理拓扑的配置、切换和监控，使物理网络从静态连线变为可编程资源。

**关键指标**：
- 拓扑热切换延迟：≤1μs（RECONFIG_ATOMIC指令→所有连线模式切换完成）
- 预设拓扑容量：≥16个完整配置
- 单节点路由表在线更新：<100ns
...
**结论**：4个原语构成Route-Transform同构验证的最小完备集。TCC-16其余7个原语（PULL/CAST/SWAP/FOLD/SCAN/MOVE/TICK）在Phase 1的SDK中以软件回退方式实现，Phase 2的ASIC流片时硬件化。
...
L4定义面向上层软件的TCC-16原语事务接口。上层通过 `tcc.fuse()`、`tcc.gemm()` 等Python API发出原语调用，L4将其封装为标准事务帧，L3通过TR-ISA管理拓扑，L2/L1负责物理传输。
...
**75%利用率为安全区间**，25%余量应对布局布线拥挤和后期功能扩展。
...
项目将交付一套完整的**上位机可视化交互大屏系统**，作为验收演示的核心亮点：

1. **拓扑实时动画**：以动画形式实时展示当前物理拓扑结构——AI模式下显示"红色蝶形归约树"，DBF模式下显示"蓝色蝶形FFT网络"。一键切换瞬间，连线颜色和操作标注同步切换，连线结构完全不变——直观证明Route-Transform同构定理。

2. **数据流可视化**：实时显示数据包在蝶形网络中的流动轨迹——AI模式展示4路特征从叶节点向根节点汇聚（AllReduce），DBF模式展示16通道数据逐级蝶形变换（FFT）。

3. **性能仪表盘**：实时显示延迟分布直方图、带宽利用率曲线、功耗热力图、切换延迟时序图。并对比呈现传统冯·诺依曼架构（仿真）在同等算力负荷下的性能断崖。

4. **原语调用追踪**：实时显示每个原语的调用频率、执行时间、拓扑切换次数，以瀑布图形式展现TCC-16原语在AI和DBF场景中的调用分布差异。
...
├─────────────────────────────────────────────────────────────────┤
│            TCCL 兼容层（NCCL/MPI/Gloo/FFTW API 映射）              │
...
| **R9** | 可选验证流片失败 | 中 | 低 | **流片不依赖核心验收**；V4核心交付均在FPGA上 |
...
**理论突破**：
- Route-Transform分解定理（Theorem 1）的实验验证，将"通信"与"计算"在数学层面统一
- Route-Transform同构定理（Theorem 2）——8对同构映射的FPGA实证，开辟"拓扑-算法联合设计"新方向
- 元拓扑-SDI化合键代数架构——3基底+5化合键的闭合代数系统，为拓扑可重构提供严格的数学基础
- 拓扑自由能原理——引入统计物理，赋予液态硬件自组织优化的理论内核
- Kronecker分形扩展——从理论上破解互连扩展墙
- TCC-16原语完备性定理（Theorem 3）的7场景覆盖验证

**学术影响**：预期在体系结构四大顶会（ISCA/HPCA/MICRO/ASPLOS）和信号处理顶刊发表论文≥5篇。

## 12.2 技术价值——建立自主标准与原语IP体系

**TCC-Link标准**：国内首个原生支持拓扑可重构指令集（TR-ISA）的芯粒互连标准。策略性选择L1/L2兼容UCIe，L3/L4实现范式级创新。

**"3+1"验证IP核库**：经FPGA充分验证的tcc.FUSE/GEMM/MAPS/LINK四个原语级IP核，开源≥20K行SystemVerilog代码。

**TCCL兼容层**：NCCL/MPI/Gloo/FFTW多API兼容，PyTorch/TensorFlow/JAX/MATLAB零修改对接。
...
本申报书（V4·TCC战略版）严格对齐海河实验室2026年度重大专项指南中关于"晶上网络中心计算范式架构研究与端侧原型验证"的全部要求：

| 指南核心要求 | V4响应 | 对齐程度 |
|------------|--------|---------|
| **液态硬件FPGA原型验证** | FPGA唯一验证载体；可选流片为锦上添花 | ✅ 完全对齐 |
| **兼容并超越UCIe的互连标准** | TCC-Link: L1/L2兼容UCIe 2.0 + L3(TR-ISA)/L4(原语事务)颠覆创新 | ✅ 完全对齐 |
| **"3+1"极简MVP架构** | tcc.FUSE + tcc.GEMM + tcc.MAPS + tcc.LINK | ✅ 完全对齐（TCC-16命名） |
| **计算与信号处理双场景验证** | 场景A(AI) + 场景B(DBF+同构演示) + 场景C(切换压测) + 可视化大屏 | ✅ 超出 |
| **NCCL/CUDA生态适配** | TCCL兼容层: NCCL/MPI/Gloo/FFTW全API兼容 | ✅ 超出 |
| **开源生态与孵化基础** | ≥20K行RTL + SDK + CI/CD + DSE工具 + 社区运营 | ✅ 完全对齐 |
| **经费2000万元** | 详列人员/平台/EDA/流片/软件/交流 | ✅ 完全对齐 |
| **SDSoW先导与奠基** | 明确定位为SDSoW战略先导，Phase 1→2→3三阶段演进 | ✅ 战略级对齐 |
| **理论基础深度** | 增补元拓扑代数+自由能原理+分形扩展，理论体系完备 | ✅ 超出 |
...
**Tags:** #NaaS #StrategicProposal #TCC #SDSoW #SDI #FPGA #LiquidHardware #Chiplet #TCC-16
