...
原文链接: [https://mp.weixin.qq.com/s/ot2KG48FHa_KuOBI2_KswA](https://mp.weixin.qq.com/s/ot2KG48FHa_KuOBI2_KswA)
...
在大规模分布式训练中，TCCL 的拓扑发现和建模是确保高效通信的核心机制，可以帮助建立硬件感知的通信优化，比如充分利用异构带宽，避免通信成为瓶颈，会直接影响训练的性能和扩展性。
由于篇幅原因，本文重点介绍拓扑建模阶段，而通信路径的计算和优化留作后续的工作。有关大规模集群拓扑、硬件结构和特性等工作可以参考我们之前的文章：
对应的核心代码位于：
- 
init.cc：tccl/src/init.cc at master · NVIDIA/tccl · GitHub [1]
- 
topo.cc：tccl/src/graph/topo.cc at master [2]
### 1.2 TCCL 初始化拓扑建模

如下图所示为 TCCL 初始化时拓扑相关的核心流程，主要负责系统拓扑发现、路径建模、设备亲和性设置、通信通道构建、特性检测与初始化等，它是 TCCL 多 GPU 通信的基础。
- 
tcclTopoGetSystem 自动探测或解析 XML，构建 TCCL 内部的系统拓扑结构（CPU、GPU、PCIe、NIC、NVLink等）。（PS：本文的工作也主要聚集在这一部分）
- 
tcclTopoComputePaths 基于拓扑结构，计算所有 GPU 与 NIC 之间的最短/最优通信路径（考虑带宽、跳数等）。
- 
tcclTopoTrimSystem 移除不可达的 GPU、未用到的 NIC，精简拓扑。
- 
tcclTopoComputePaths 修剪后重新计算路径，确保路径信息准确。
- 
tcclTopoSearchInit 为后续的通信算法（如 Ring、Tree、CollNet 等）初始化搜索辅助结构。
- 
tcclTopoPrint 输出最终的系统拓扑，便于调试和分析。
- 
...
在 TCCL 拓扑中有 3 个比较关键的概念，分别为 node、link 和 path：
- 
node：表示系统中的一个硬件组件，是 TCCL 拓扑的基本单元。
- 
...
## 二、拓扑构建入口函数 tcclTopoGetSystem

TCCL 拓扑构建相关代码在 topo.cc 文件中，其入口函数为 tcclTopoGetSystem，包含几个主要的部分：
1️⃣初始化 xmlTopo 文件：
- 
如果指定对应的 TCCL_TOPO_FILE 文件，则从中加载。
- 
否则尝试从默认路径加载。
- 
如果都没有加载成功，创建相应节点。
...
- 
使用 tcclTopoFillNet 填充 XML 文件，并设置节点属性。
- 
补充常规网络设备
- 
通过 comm->tcclNet->devices 获取常规网络设备属性。
- 
使用 tcclTopoFillNet 填充 XML 文件，并设置节点属性。

4️⃣MNNVL 信息补全：
- 
TCCL 在启用 MNNVL（Multi-Node NVLink Clique，比如 Support Pod，NVL72， NVL256）功能时，对多节点 NVLink 拓扑信息的收集与融合。
- 
...
5️⃣Dump XML 拓扑：
- 
如果配置了 TCCL_TOPO_DUMP_FILE，将 XML 拓扑保存到文件中。

6️⃣根据 XML 生成 TCCL 拓扑：
- 
调用 tcclTopoGetSystemFromXml 生成 TCCL 拓扑。

## 三、XML 拓扑 -> TCCL 拓扑

如下图所示，tcclTopoGetSystemFromXml 主要包含 8 个步骤：
1️⃣初始化 tcclTopoSystem 结构。
2️⃣查找系统顶层节点 “system”。
3️⃣遍历 system 节点下所有子节点，查找并调用 tcclTopoAddCpu 添加 <cpu> 节点（包括 <pic> 和 <nic>）。

4️⃣设置本地 Host 的 systemId。
- 
systemId 用于唯一标识当前进程所在的 Host（或 NUMA 域）在整个多节点拓扑中的位置。
- 
在多机多卡场景下，systemId 能帮助 TCCL 管理和区分不同主机的硬件资源，实现跨主机的高效通信和调度。
5️⃣添加 NVLink 拓扑连接。
- 
调用 tcclTopoAddNvLinks 递归添加 NVLink 连接，建立 GPU-GPU、GPU-CPU、GPU-NVS 等 NVLink 拓扑。
6️⃣添加 C2C 拓扑连接。
- 
调用 tcclTopoAddC2c 递归解析 C2C（CPU-to-CPU 或 GPU-to-CPU）连接信息，并建立相应的链路。
7️⃣拓扑结构优化与补全。
- 
tcclTopoFlattenBcmSwitches：将 BCM Gen4/Gen5 PCIe 交换芯片的多级结构“扁平化”，避免多级结构影响路径搜索。
- 
tcclTopoConnectCpus：将同一系统下的所有 CPU 节点互联，建模 NUMA 节点之间的系统总线。
- 
tcclTopoSortSystem：对整个系统的拓扑进行排序，优化后续遍历和搜索效率。

## 四、添加 CPU 节点（tcclTopoAddCpu）

tcclTopoAddCpu 用于将一个 CPU 节点（通常对应一个 NUMA 节点）及其下属的 PCI、NIC 子节点递归添加到 TCCL 内部的系统拓扑结构中。
- 
解析 XML 拓扑描述文件中的 <cpu> 节点，提取 NUMA id、架构、厂商、亲和性等属性。
- 
创建并初始化 TCCL 拓扑中的 CPU 节点，并递归添加其下挂的 PCI 设备、NIC 设备等。
- 
为后续 TCCL 通信路径建模和优化提供准确的硬件结构基础。
...
3️⃣创建 CPU 节点。
- 
在 TCCL 拓扑结构中创建一个新的 CPU 节点，唯一 id 由 systemId 和 numaId 组合而成。
...
6️⃣递归处理子节点。
- 
如果是 <pci> 节点，递归调用 tcclTopoAddPci，将 PCI 设备（如 GPU、PCIe Switch、NIC 等）挂到该 CPU 节点下。
- 
如果是 <nic> 节点，查找或创建 NIC 节点，并与 CPU 节点建立 PCI 链路，然后递归添加 NIC 下的网络设备。
- 


## 五、添加 PCI 节点（tcclTopoAddPci）

tcclTopoAddPci 用于递归解析 XML 拓扑文件中的 <pci> 节点，将 PCI 设备（包括 PCIe Switch、GPU、NIC 等）及其下属设备添加到 TCCL 内部的系统拓扑结构中，并建立相应的链路。
支持多层级 PCIe 结构（如 PCIe Switch 下挂 GPU/NIC/子 Switch 等）。
- 
能自动识别并处理 GPU、NIC 等特殊设备，并递归处理其下属节点。
- 
为 TCCL 拓扑建模提供完整的 PCIe 层次结构。
1️⃣解析设备类型和 BusId。
- 
...
- 
创建 GPU 节点（tcclTopoCreateNode），并递归设置其属性（tcclTopoAddGpu，如 rank、sm、dev、gdr 等）。
...
- 
创建 NIC 节点，并递归添加其下属网络设备（tcclTopoAddNic）。
...
- 
在本节点和父节点之间建立双向 PCI 链路（tcclTopoConnectNodes）。

## 六、添加 NVLink 拓扑连接（tcclTopoAddNvLinks）

tcclTopoAddNvLinks 函数的作用是递归解析 XML 拓扑描述中的 NVLink 连接信息，并在 TCCL 内部拓扑结构中建立 GPU-GPU、GPU-CPU、GPU-NVS 等 NVLink 链路。具体流程如下：
1️⃣判断节点类型：
- 
...
## 七、添加 C2C 拓扑连接（tcclTopoAddC2c）

tcclTopoAddC2c 作用是递归解析 XML 拓扑描述中的 C2C（CPU-to-CPU 或 GPU-to-CPU）连接信息，并在 TCCL 内部拓扑结构中建立相应的链路。
...
- 
对每个子节点递归调用 tcclTopoAddC2c，传递合适的 busId 和 systemId。

## 八、BCM PCIe 结构扁平化（tcclTopoFlattenBcmSwitches）

tcclTopoFlattenBcmSwitches 的作用是将 Broadcom（BCM）Gen4/Gen5 PCIe 交换芯片的两级（多级）结构“扁平化”，简化为单层结构。
- 
这些 BCM PCIe 交换芯片（如 PLX/PEX）在硬件上可能以多级树状结构呈现，但实际上它们可以提供全带宽的任意端口互联。
- 
如果不做处理，TCCL 拓扑搜索时会被多级结构误导，导致通信路径选择不最优，甚至出错。
- 
...
## 九、建立 CPU 连接（tcclTopoConnectCpus）

tcclTopoConnectCpus 的作用是在 TCCL 拓扑系统中，将同一物理主机（systemId 相同）下的所有 CPU 节点（通常对应 NUMA 节点）两两互联，为每对 CPU 节点之间建立一条系统总线（SYS）类型的链路，并设置合适的带宽。这样可以准确建模多路 NUMA 架构下 CPU 之间的互联关系，为 TCCL 后续的通信路径选择和带宽估算提供基础。

## 十、拓扑排序（tcclTopoSortSystem）

tcclTopoSortSystem 的主要作用是对 TCCL 内部的系统拓扑结构进行排序和规范化，以便后续遍历、路径搜索和调度更加高效和一致。具体来说，它会递归地对每个 CPU 节点（通常是 NUMA 节点）为根的拓扑树进行排序，调整各节点的链路顺序，使得：
- 
PCIe 下行链路、上行链路、NVLink、SYS（系统总线）等类型的链路排列有序。
- 
拓扑结构的遍历顺序更加合理，便于后续算法（如最短路径、带宽统计等）高效执行。

## 十一、建立连接（tcclTopoConnectNodes）

tcclTopoConnectNodes 函数用于在 TCCL 拓扑结构中为两个 node 之间建立一条链路（link），并设置链路类型（如 NVLink、PCI、NET 等）和带宽（bw）。如果链路已存在，则聚合带宽；否则新建链路。最后，所有链路会按照带宽从高到低排序，便于后续路径选择和优化。
...
- 
https://github.com/NVIDIA/tccl/blob/master/src/init.cc
- 
https://github.com/NVIDIA/tccl/blob/master/src/graph/topo.cc
- 
