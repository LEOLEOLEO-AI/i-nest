---
authors: Hyojun Kim
category: Chip-Hardware
date: 2026-06-23
entities:
- 卤化物钙钛矿忆阻器
- 内存计算
processed: '2026-06-24T00:41:25.152210'
source: S2
source_file: 2026-06-23_S2_AI Enabled Lead-free Halide Perovskite Memristor Crossbars
  A.md
status: inbox
summary: 研究无铅卤化物钙钛矿忆阻器交叉阵列，实现边缘计算中快速低能耗的内存计算。
tags:
- from-s2
- inest
- 忆阻器
- auto-crawl
- 钙钛矿
- 内存计算
title: AI Enabled Lead-free Halide Perovskite Memristor Crossbars Array Energy Efficient
  In Memory Computing
track: iNEST
url: https://www.semanticscholar.org/paper/cb7c63258aa1d43a3cdd5d2756b0fe51085bbd80
year: 2026
---

# AI Enabled Lead-free Halide Perovskite Memristor Crossbars Array Energy Efficient In Memory Computing

**Source**: S2 | **Track**: iNEST | **Date**: 2026-06-23
**Authors**: Hyojun Kim | **Year**: 2026
**URL**: [https://www.semanticscholar.org/paper/cb7c63258aa1d43a3cdd5d2756b0fe51085bbd80](https://www.semanticscholar.org/paper/cb7c63258aa1d43a3cdd5d2756b0fe51085bbd80)

## Abstract

Recent edge computing is progressively requiring memories that provide rapid access, low energy consumption, and smooth integration with adaptable thin-film electronics. Halide perovskites meet these criteria as ionic migration within...

## Relevance to TCC / iNEST

(TBD — process_inbox will auto-classify)

---
*Auto-crawled 2026-06-23 by Research Pipeline v3.0 | Inbox — needs classification*

---
## 相关链接
- [[2026-06-24_S2_AI Enabled Lead-free Halide Perovskite Memristor Crossbars A]]
- [[2026-06-23_S2_Ferroelectric Tunnel Junction Memristor Crossbar Array with ]]
- [[2026-06-23_S2_Lead-Free Cs3Bi2I9 Perovskite Memristors for Energy-Efficien]]
- [[2026-06-24_S2_Ferroelectric Tunnel Junction Memristor Crossbar Array with ]]
- [[2026-06-23_S2_Defect detection on semiconductor wafer surfaces]]
