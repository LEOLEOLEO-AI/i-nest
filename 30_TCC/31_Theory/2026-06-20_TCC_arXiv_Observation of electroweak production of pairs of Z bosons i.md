---
title: "Observation of electroweak production of pairs of Z bosons in proton-proton coll"
date: 2026-06-20
track: TCC
source: arXiv
authors: 
year: 2026
url: http://arxiv.org/abs/2606.20558v1
citations: 
tags: [from-arxiv, inbox, tcc, electroweak, Z boson, CMS, classified]
provenance: external
---

# Observation of electroweak production of pairs of Z bosons in proton-proton coll

**来源**: arXiv
**作者**: 
**年份**: 2026
**引用数**: 

## 摘要

The first evidence of electroweak (EW) production of pairs of Z bosons in association with two jets (jj) in the final state ZZjj $\to$ $\ell\ellνν$jj, where $\ell$ = e, $μ$, is reported by the CMS experiment. The analysis is based on a data sample of proton-proton (pp) collisions at $\sqrt{s}$ = 13 

## 链接

[http://arxiv.org/abs/2606.20558v1](http://arxiv.org/abs/2606.20558v1)

---
*自动抓取于 2026-06-20 10:10 | 待分类处理*

---
## 相关链接
- [[2026-06-20_iNEST_arXiv_MemoryWAM Efficient World Action Modeling with Persistent Me]]
- [[2026-06-20_TCC_arXiv_Incorporating physical source parameters into microlensing m]]
- [[2026-06-22_TCC_arXiv_Optimal Deterministic Multicalibration and Omniprediction]]
- [[2026-06-23_TCC_arXiv_Optimal Deterministic Multicalibration and Omniprediction]]
- [[2026-06-20_TCC_arXiv_Thinking in Boxes 3D Editing in Real Images Made Easy]]
