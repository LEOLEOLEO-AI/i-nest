...
1. **Complete CST-optimal topology mapping**: For each of the six NCCL-standard collective primitives, we derive the closed-form CST-optimal physical topology, prove its optimality, and provide the in-network compute decomposition that allows computation to occur at interconnect nodes rather than endpoint GPUs.
