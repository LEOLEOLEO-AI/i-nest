...
# ============ v25: Sensorimotor Loop parameters ============
SM_ERROR_WEIGHT = 0.3      # How strongly motor error modulates FEP
SM_READOUT_TAU = 10.0      # Temporal smoothing for motor readout
SM_N_PATTERNS = 10         # Number of sensorimotor patterns
...
class SDI_v24:
    def __init__(self):
        ...
        # Structured input
        self.structured_gen = StructuredSpikeGen(len(self.sensor_idx), N_patterns=SM_N_PATTERNS)

        # v25: Sensorimotor loop state
        self.motor_readout = np.zeros(len(self.motor_idx)) if len(self.motor_idx) > 0 else np.zeros(1)
        self.motor_target = np.zeros(len(self.motor_idx)) if len(self.motor_idx) > 0 else np.zeros(1)
        self.motor_error = 0.0
        self.motor_error_prev = 0.0
        self.reward_factor = 1.0
        self.motor_error_hist = []
        if len(self.motor_idx) > 0:
            self.sm_pattern_map = {}
            for p in range(SM_N_PATTERNS):
                rng = np.random.RandomState(p + 1000)
                self.sm_pattern_map[p] = rng.rand(len(self.motor_idx)) > 0.5
        else:
            self.sm_pattern_map = {}
        ...
        for pre in active_nodes:
            ...
            for j in self.adj[pre]:
                ...
                if active_mask[post]:
                    ...
                    dw = ETA_LTP * fep_boost * self.reward_factor
                    ...
                    dw = -ETA_LTD * fep_suppress * self.reward_factor
                    self.weight[j] = np.clip(self.weight[j] + dw, 0.01, 3.0)
    ...
    def step(self):
        ...
        # v25: Sensorimotor readout and error
        self._sensorimotor_readout()
        self._sensorimotor_error()

        # v24: FEP energy computation (before STDP so signals available)
        self.fep_compute_energy()
        # v25: Modulate FEP with motor error
        self._sensorimotor_modulate_fep()
    ...
    # ===== v25: Sensorimotor Loop =====
    def _sensorimotor_readout(self):
        if len(self.motor_idx) == 0:
            self.motor_readout = np.zeros(1)
            return
        motor_act = self.h[self.motor_idx]
        self.motor_readout = self.motor_readout * 0.9 + motor_act * 0.1
        self.motor_readout = (self.motor_readout > 0.3).astype(np.float64)

    def _sensorimotor_error(self):
        if len(self.motor_idx) == 0:
            self.motor_error = 0.0
            return
        pattern_id = self.structured_gen._last_pattern
        if pattern_id in self.sm_pattern_map:
            self.motor_target = self.sm_pattern_map[pattern_id].astype(np.float64)
        self.motor_error = np.mean(np.abs(self.motor_readout - self.motor_target))
        self.motor_error_hist.append(self.motor_error)
        if len(self.motor_error_hist) > 100:
            self.motor_error_hist.pop(0)

    def _sensorimotor_modulate_fep(self):
        if len(self.motor_idx) == 0:
            self.reward_factor = 1.0
            return
        avg_error = np.mean(self.motor_error_hist[-10:]) if self.motor_error_hist else self.motor_error
        boost = 1.0 + SM_ERROR_WEIGHT * avg_error
        # Boost FEP on interneurons carrying sensor->motor signals
        mask = np.ones(self.N, dtype=bool)
        mask[self.sensor_idx] = False
        mask[self.motor_idx] = False
        self.F_local[mask] *= boost
        # v25.1: Reward-modulated coupling
        # If error is decreasing, reward=boost STDP. If increasing, punish=suppress.
        delta_err = self.motor_error_prev - self.motor_error
        self.reward_factor = 1.0 + 2.0 * delta_err  # [-1, +3] range
        self.reward_factor = np.clip(self.reward_factor, 0.5, 2.5)
        self.motor_error_prev = self.motor_error
