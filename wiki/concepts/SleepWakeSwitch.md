---
title: "SleepWakeSwitch"
type: concept
domain: iNEST
created: 2026-08-23
auto: true
---

# SleepWakeSwitch

**Domain**: iNEST
**First mentioned**: auto-extracted
**Last updated**: 2026-08-23

## Definition
A bistable neural circuit that flips between mutually inhibitory sleep-promoting and wake-promoting neuron populations, ensuring discrete behavioral states. Serves as a canonical example of a robust bistable switch, useful for designing stable state machines in TCC for fault-tolerant network reconfiguration.

## Context
Auto-extracted concept from raw material compilation.

## Related Work

[[AINext_Initiative]]
[[AcceleratorReconfigurationOverhead]]
[[Accessibility_Plasticity]]
[[Active_Inference_Topology]]
[[AdaptiveArchitecture]]
[[AdaptiveCombinerWeighting]]
[[Adaptive_Service_Reconfiguration]]
[[Additive_Manufacturing_Supply_Chain]]
[[AmoebaInspiredLocomotion]]
[[Anomaly_Protected_Scaling_Law]]
[[BayesianConnectivityEstimation]]
[[CriticalityDrivenStateTransition]]
[[Criticality]]
[[InNetworkReconfiguration]]
[[Interconnect_Routing]]
[[JointFlightRoutingOptimization]]
[[LLMDrivenPathSelection]]
[[MeasurementInducedLinearity]]
[[MoE_Routing]]
[[Network_Topology_Design]]
[[OddConservedDynamics]]
[[Self_Organized_Criticality]]
[[Synaptic_Plasticity]]
[[TCC]]
[[iNEST]]

## Sources
- See wiki/articles/ for source article summaries

## Open Questions
- *(Explore connections to other concepts)*
