---
title: "CriticalExponentGamma"
type: concept
domain: Cross
created: 2026-08-21
auto: true
---

# CriticalExponentGamma

**Domain**: Cross
**First mentioned**: auto-extracted
**Last updated**: 2026-08-21

## Definition
The exponent γ in the susceptibility relation χ = ∂φ/∂h|h=0 ∝ |r|^(−γ), where r ∝ (T−Tc) measures distance from the critical point. Quantifies how sensitively the iNEST system responds to external stimuli near criticality, informing optimal operating points for computational efficiency.

## Context
Auto-extracted concept from raw material compilation.

## Related Work

[[2026-06-15_大脑的量子纠缠与信息动力学]]
[[AINext_Initiative]]
[[ANN_SNN_WeightSpaceGap]]
[[AcousticAnomalyDetection]]
[[ActivityAvalanche]]
[[AdversarialSpikePerturbation]]
[[Agent_Interaction_Sequencing]]
[[AnalogDomainCompression]]
[[Automated_Reward_Shaping]]
[[AvalancheBranchingRatio]]
[[Avalanche_Dynamics]]
[[Avoided_Criticality]]
[[Baseline_Change_Record]]
[[BioInspiredInterceptionTask]]
[[BioInspiredInterception]]
[[BranchingRatio]]
[[BulletTrainScheduling]]
[[CoDesigned_SNN]]
[[CoDesigned_SNN_Online_Learning]]
[[CoOptimizedMappingScheduling]]
[[Continuous_Control_Spiking_Stability]]
[[ConventionalAndSpikingDualMode]]
[[CriticalityInducedComputationalCapacity]]
[[Criticality]]
[[SNN]]
[[Self_Organized_Criticality]]
[[Spiking_Neural_Network]]
[[iNEST]]

## Sources
- See wiki/articles/ for source article summaries

## Open Questions
- *(Explore connections to other concepts)*
