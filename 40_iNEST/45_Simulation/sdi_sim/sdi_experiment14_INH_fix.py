"""
SDI 实验十四 — INH_RATIO修正，解决λΦ过饱和
单一参数修改：INH_RATIO 7→13
目标：激活率<20%，λ→0.93，Φ→0.75，CST对齐论文
完全继承实验十三，只改INH_RATIO
"""
...
BASE    = '/vault/sdi_sim'
OUT     = os.path.join(BASE, 'exp14_INH_fix_results.json')
...
EI_RATIO=0.20
INH_RATIO=13.0   # ← 7→13（Brunel 2000上界）
...
def build_Wei(W,ei):
    Wei=W.copy(); Wei[ei==-1,:]*=-INH_RATIO; return Wei

def activate_EI(W,Wei,h,tau,rng):
    N=W.shape[0]; leak=(1.0-1.0/tau).astype(np.float32)
    drive=np.zeros(N,dtype=np.float32)
    drive[rng.choice(N,max(3,int(N*0.05)),replace=False)]=rng.uniform(0.2,0.6,max(3,int(N*0.05)))
    h_new=leak*h+np.tanh(Wei@h+drive)
    h_new=np.where(h_new>0.1,h_new,0.0)
    return np.clip(h_new,0,1).astype(np.float32)
...
def run(name,cfg,seed):
    ...
    TH=Theta(tau); Wei=build_Wei(W,ei)
    n_inh=(ei==-1).sum()
    print(f"  E/I:{N-n_inh}E+{n_inh}I  INH={INH_RATIO}×  Θ={TH:.3f}")
    ...
    for step in range(N_STEPS):
        h=activate_EI(W,Wei,h,tau,rng); act=h.copy()
        ema=0.97*ema+0.03*act; hist.append(act.copy())
        if len(hist)>50: hist.pop(0)
        W,EL,ltp,ltd=rule1(W,EL,ltp,ltd,act)
        if step%LTP_DECAY_INT==0: ltp=np.maximum(ltp-1,0)
        if step%GROW_INT==0:
            W=rule2(W,EL,ema,rng); Wei=build_Wei(W,ei)
        if step%SCALING_INT==0:
            W=rule3(W,ema); Wei=build_Wei(W,ei)
        if step%PRUNE_INT==0:
            W=rule4(W,EL,ema,rng); Wei=build_Wei(W,ei)
        if step%LOG_INT==0:
            Sc=compute_Sc(W,rng)
            lm=lam_eff(hist); ph=phi_eff(hist)
            ps=psi_eff(W,W0)
...
if __name__=='__main__':
    ...
    print("="*65)
    print(f"SDI 实验十四 — INH_RATIO={INH_RATIO}（7→13，Brunel 2000上界）")
    print(f"  目标：激活率<20%，λ→0.93，CST对齐论文")
    print("="*65)
    results=[]
    ...
    for net,ref in REF.items():
        if net in summary:
            ...
            print(f"  {net}: 仿真={cst:.3f}  论文={ref}  "
                  f"比值={cst/ref:.2f}×  act={act*100:.0f}%  [{summary[net]['IIL']}]")
    print("\n【vs 实验十三对比】")
    e13={'WS_300':1.518,'C_elegans':1.089,'Human_HCP':2.299}
    e13_act={'WS_300':0.87,'C_elegans':0.82,'Human_HCP':0.87}
    for net,s in summary.items():
        c13=e13.get(net,0); a13=e13_act.get(net,0)
        print(f"  {net}: act {a13*100:.0f}%→{s['act']*100:.0f}%  "
              f"CST {c13:.3f}→{s['CST']:.3f}  [{s['IIL']}]")
