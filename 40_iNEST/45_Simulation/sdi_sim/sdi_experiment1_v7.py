...
SPECIES = {
    ...
    'Rat_Cortex':     {'N':73,  'k':14, 'k_init':6,  'p_init':0.12, 'sf':0.15, 'init':'ba',
        'bio':{'sigma':3.,'C':.42,'L':1.9,'alpha':2.,'el':.18},
        'tgt':{'sigma':(2.5,None),'C':(.30,None),'L':(1.5,3.),'alpha':(1.5,2.5),'el':(.15,.28)}},
    'Mouse_Cortex':   {'N':112, 'k':14, 'k_init':6,  'p_init':0.12, 'sf':0.15, 'init':'ba',
...
def main():
    ...
    def fix(o):
        ...
        return o
    with open(f'{OUT}/exp1_v7_results.json','w') as f:
    ...
    plt.tight_layout()
    plt.savefig(f'{OUT}/exp1_v7_convergence.png',dpi=130,bbox_inches='tight')
    plt.close()
