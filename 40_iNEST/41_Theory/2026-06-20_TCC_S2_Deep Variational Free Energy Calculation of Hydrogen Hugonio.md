---
title: "Deep Variational Free Energy Calculation of Hydrogen Hugoniot"
date: 2026-06-20
track: TCC
source: S2
authors: Zihan Li, Hao Xie, Xinyang Dong, Lei Wang
year: 2025
url: https://www.semanticscholar.org/paper/2b6af51a937df3579d3ab9d81bef8fdca43a14f4
citations: 4
tags: [deep variational free energy, hydrogen Hugoniot, equation of state, warm dense matter, normalizing flow, autoregressive transformer, permutational equivariant flow, classified]
provenance: external
---

# Deep Variational Free Energy Calculation of Hydrogen Hugoniot

**来源**: S2
**作者**: Zihan Li, Hao Xie, Xinyang Dong, Lei Wang
**年份**: 2025
**引用数**: 4

## 摘要

We develop a deep variational free energy framework to compute the equation of state of hydrogen in the warm dense matter region. This method parameterizes the variational density matrix of hydrogen nuclei and electrons at finite temperature using three deep generative models: a normalizing flow model for the Boltzmann distribution of the classical nuclei, an autoregressive transformer for the distribution of electrons in excited states, and a permutational equivariant flow model for the unitary

## 链接

[https://www.semanticscholar.org/paper/2b6af51a937df3579d3ab9d81bef8fdca43a14f4](https://www.semanticscholar.org/paper/2b6af51a937df3579d3ab9d81bef8fdca43a14f4)

---
*自动抓取于 2026-06-20 10:10 | 待分类处理*


<!-- orphan-cleanup: linked to MOC -->
## 来源回链

- [[iNEST_Master_Index]]
