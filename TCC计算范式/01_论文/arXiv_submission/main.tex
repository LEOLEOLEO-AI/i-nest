...
\begin{equation}
\Sc = (C \cdot H \cdot M \cdot R_{sw})^{1/4} \tag{2}
\end{equation}
where $C$ = global connectivity (LCC fraction); $H$ = hierarchical depth (scale-normalized $k$-core ratio~\cite{seidman1983}); $M$ = resolution-corrected modularity $Q'$~\cite{fortunato2007}; $R_{sw}$ = small-world coefficient (tanh-normalized Watts--Strogatz $\sigma$~\cite{watts1998}).
...
\textbf{Spatial complexity:} $\Sc = (C \cdot H \cdot M \cdot R_{sw})^{1/4}$, where:
\begin{itemize}
\item $C = |LCC|/N$ (global connectivity; bounded $[0,1]$ by construction)
\item $H = \min[(k_{\max} / k_{\mathrm{null}}) / 6.667,\; 1.0]$; $k_{\mathrm{null}} \approx \ln(N)/\ln(\ln(N))$; anchor: Human HCP ($H = 1.0$)
\item $M = \max[(Q - 0.02) / (1 - 0.02),\; 0.01]$; $Q$ = Louvain modularity (100 random restarts, $\gamma = 1.0$)
\item $R_{sw} = \tanh[(\sigma - 1) / 2]$; $\sigma = (C/C_{\mathrm{rand}})/(L/L_{\mathrm{rand}})$
\end{itemize}
