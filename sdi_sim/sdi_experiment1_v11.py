...
def main():
    ...
    def fix(o):
        ...
        return o
    with open(f'{OUT}/exp1_v11_results.json','w') as f:
        json.dump(fix(all_r), f, indent=2)
    print(f'\nResults → exp1_v11_results.json')
    ...
    plt.suptitle('SDI Experiment 1 v11 — 7-Species Cross-Kingdom Universality\n'
                 f'(5 random seeds each | neuron-level + mesoscale★ | Hill MLE α)',
                 fontsize=11, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'{OUT}/exp1_v11_convergence.png', dpi=130, bbox_inches='tight')
    plt.close()
    print('Plot → exp1_v11_convergence.png')
    print('DONE')
