#!/usr/bin/env python3
"""
SDI 实验一 v16 扩展新物种验证普适性 — +4物种，共14物种
...
版本历史:
  ...
  v16: 在v15基础上扩展4个新物种（共14物种）：
       Marmoset★ (Majka et al. 2020 Sci Data, 114脑区)
       Pigeon★ (Shanahan et al. 2013 PLOS CB, 45脑区)
       Honeybee★ (Menzel 2012 Nat Rev Neurosci, 160节点)
       Starfish_larva (Randel 2014 eLife, N=174, neuron级)
       目的：进一步验证SDI极简规则的跨物种、跨门级普适性
...
SPECIES = {
    ...
    # ---- 新增4物种 (v16) ----
    # 狨猴★：Majka et al. 2020 Scientific Data, 114脑区mesoscale connectome
    'Marmoset★': {
    ...
    # 鸽子★：Shanahan et al. 2013 PLOS CB, 45脑区mesoscale
    'Pigeon★': {
        'N':45,'k':12,'k_init':8,'p_init':0.10,'sf':0.18,'level':'mesoscale',
        'bio':{'sigma':1.50,'C':0.49,'L':1.9,'alpha':2.0,'el':0.18},
        'ref':'Shanahan et al. 2013 PLOS CB; pigeon telencephalon connectome',
        'tgt':{'sigma':(1.2,None),'C':(0.25,None),'L':(1.5,3.0),'alpha':(2.0,4.5),'el':(0.15,0.28)}},

    # 蜜蜂★：Menzel 2012, 160节点蘑菇体+嗅觉回路 mesoscale
    'Honeybee★': {
    ...
    # 海星幼虫：neuron级，Randel 2014 eLife, N=174，完整幼虫神经系统connectome
    'Starfish_larva': {
...
def main():
    print(f'SDI 实验一 v16 扩展新物种验证普适性 (+4物种，共14物种)')
    print(f'BTW_MODE={BTW_MODE}, BTW_DRIVE_N={BTW_DRIVE_N}, BTW_DRIVE_INTERVAL={BTW_DRIVE_INTERVAL}')
    print(f'N_STEPS={N_STEPS}')
    print(f'新增物种：Marmoset★, Pigeon★, Honeybee★, Starfish_larva')
    all_r = {}; t0 = time.time()
    ...
    print(f'\n{"="*58}')
    print(f'ALL DONE  {elapsed:.1f}s  ({len(SEEDS)} seeds × 14 species)')
    print('='*58)
    ...
    def fix(o):
        ...
        return o
    with open(f'{OUT}/exp1_v16_results.json','w') as f:
        json.dump(fix(all_r), f, indent=2)
    print(f'\nResults → exp1_v16_results.json')
    
    # 绘图 14行×5列
    fig, axes = plt.subplots(14, 5, figsize=(22, 38))
    ...
    plt.suptitle(f'SDI Experiment 1 v16 — 14 Species Universality | BTW_interval={BTW_DRIVE_INTERVAL} | N_STEPS={N_STEPS}',
                 fontsize=11, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'{OUT}/exp1_v16_convergence.png', dpi=130, bbox_inches='tight')
    plt.close()
    print('Plot → exp1_v16_convergence.png')
    print('DONE')
