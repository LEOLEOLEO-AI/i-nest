...
本研究为TCC（Topology-Centric Computing，拓扑中心计算）范式提供了系统性的计算实证，支持"大道至简"的核心学术信仰：极简规则集通过与环境的能量交互，能够自发涌现神经系统级别的复杂功能。
...
**实验五（v14）**：引入BTW驱动-耗散机制，验证α能否系统性降至[1.5, 2.5]范围内。同时引入空间约束（神经元位置影响连接概率），研究几何因素对小世界涌现的影响。
...
$$\text{随机网络} \xrightarrow{\text{STDP+WS重连}} \text{小世界拓扑（}\sigma>3\text{）} \xrightarrow{\text{突触缩放}} \text{稳定临界态（}\alpha\approx 2\text{）} \xrightarrow{\text{竞争修剪}} \text{模块化（}Q>0.6\text{）} \xrightarrow{\text{真实connectome}} \text{功能性嗅觉编码（KC 2.55\%）}$$

四条极简规则的每一条都有清晰的生物学对应（STDP/Hebbian学习、轴突发芽、稳态可塑性、突触修剪），且全物种统一、无需物种特异调参。这正是"大道至简"的计算形式化：

> **简单性之"一"** = SDI四规则（STDP + WS重连 + 突触缩放 + 竞争修剪）  
> **能量输入** = 结构化外部刺激  
> **涌现复杂性** = 小世界拓扑 + 临界动力学 + 模块化结构 + 嗅觉编码  

实验一（10物种跨界验证）和实验四（模块化涌现）共同证明了规则集的**普适性**；实验二（Hemibrain功能验证）证明了其**功能性**；实验三（规则边界发现）证明了其**最小性**（缺一不可）。
