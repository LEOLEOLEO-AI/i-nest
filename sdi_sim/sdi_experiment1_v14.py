#!/usr/bin/env python3
"""
SDI 实验一 v14 BTW慢驱动 — 修复alpha偏高问题

版本历史:
  v13 FINAL: 10物种达标，但alpha普遍偏高（神经元级3.0-3.7，目标[1.5,2.5]）
  v14: 引入BTW（Bak-Tang-Wiesenfeld 1987）慢驱动机制，使网络维持在真正SOC临界态
       期望alpha降低至生物真实值[1.5,2.5]附近
...
MAX_FIX=8; N_STEPS=8000; LOG_INT=500; P_REWIRE=0.15; REWIRE_INT=50
...
BTW_DRIVE_INTERVAL = 3   # 每3步驱动一次（不是每步）
...
def main():
    ...
    # v14 BTW驱动说明
    print('\n【v14 BTW慢驱动说明】')
    print(f'  1. BTW_MODE={BTW_MODE}: 启用Bak-Tang-Wiesenfeld 1987慢驱动机制')
    print(f'  2. 每{BTW_DRIVE_INTERVAL}步激活{BTW_DRIVE_N}个随机节点（vs v13约20%节点）')
    print(f'  3. N_STEPS={N_STEPS}（vs v13: 5000）—— 更多步让网络演化到临界态')
    print(f'  4. 分离时间尺度：驱动速率远小于松弛速率 → 期望产生幂律alpha→1.5')
    print(f'  5. 突触缩放阈值按BTW模式调整：过度激活>10%↓，低激活<0.5%↑')
    print('\n【数据诚信说明（继承v13）】')
    ...
    plt.suptitle('SDI Experiment 1 v14 BTW慢驱动 — 10-Species Cross-Kingdom Universality\n'
                 f'(5 random seeds each | BTW_N={BTW_DRIVE_N}/interval={BTW_DRIVE_INTERVAL} | N_STEPS={N_STEPS})',
