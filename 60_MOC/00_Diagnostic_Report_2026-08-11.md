...
- pipeline_guard.py ???????? `cleared` ??????????`paused` ? `requires_confirmation=true`?`--resume` ????
- ?????????cleared?completed / timeout?paused / resume?completed?
- ??? state/pipeline_pause.json ?????? idle????? 08:00 ???
- ??????? `ObsidianVaultPipeline`??? `iNEST_Daily_Pipeline`?08:00??
- ??? `iNEST_Inbox_Afternoon`?20:00??process_inbox --limit 20 + gen_insights ??????
- process_inbox.py ?? --limit ???gen_insights.py ???????????????
- Git??? 5 ? 0 ?????????????? Genspark arXiv ?? 08-04~08-10?47 ???? arxiv-auto???? github + gitee ???
- ???Obsidian Sync ???? 0 ????????? P1 ?????????Gitee ???? 3 ??????????? GC?
