---
title: "Review on tsunami research and risk mitigation: from prediction models to resilient coastal communities"
authors: ['Tianxing Ma', 'liangxu shen', 'Zejian Chen', 'Dongfang Liang']
year: 2026
doi: "10.1038/s44304-026-00195-7"
journal: "npj natural hazards."
cited_by: 4
track: iNEST
source: openalex
query: "edge of chaos reservoir computing dynamics"
date: 2026-07-18
topics: ['Tropical and Extratropical Cyclones Research', 'earthquake and tectonic studies', 'Flood Risk Assessment and Management']
---

# Review on tsunami research and risk mitigation: from prediction models to resilient coastal communities

**Tianxing Ma, liangxu shen, Zejian Chen** (2026) | *npj natural hazards.* | Cited: 4

## Abstract

Abstract Tsunami science is moving from standalone physics simulations to integrated forecasting and risk-governance frameworks. This review synthesizes advances in multi-source observations, signal detection, propagation and inundation modeling, physics–AI hybrid prediction, and real-time warning workflows. It further discusses uncertainty communication, decision support, and pathways for linking forecast systems with resilient coastal planning and community preparedness.

## Topics

Tropical and Extratropical Cyclones Research, earthquake and tectonic studies, Flood Risk Assessment and Management

## Links

- [OpenAlex](https://openalex.org/W7136380384)
- [DOI](https://doi.org/10.1038/s44304-026-00195-7) (if doi else '')
