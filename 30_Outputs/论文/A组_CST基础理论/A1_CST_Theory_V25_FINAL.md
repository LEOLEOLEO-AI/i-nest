...
1. Methods/UCCP: Replaced conflicting M/R_{sw} definitions (Theory ≠ Methods in V23) with unified Unified Cross-Species Computation Protocol (UCCP). M now consistently = resolution-corrected Louvain Q' throughout; R_{sw} = tanh-normalized σ throughout.
...
3. Methods/M: Added Q_rand = 0.02 correction and floor ε = 0.01 to prevent Sc = 0 for near-random networks (E. coli, MLP).
